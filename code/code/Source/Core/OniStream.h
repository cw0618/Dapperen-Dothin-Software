...
public:
	...
	OniStatus convertC2DCoordinates(int colorX, int colorY, OniDepthPixel depthZ, int* pDepthX, int* pDepthY);
	OniStatus convertD2CCoordinates(int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY);

	void filterSpeckles(void* srcbuf, int newVal, int maxSpeckleSize, int maxDiff);
