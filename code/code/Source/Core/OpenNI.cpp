...
{
    g_Context.clearErrorLogger();
    stream->pStream->filterSpeckles(buf, newVal, maxSpeckleSize, maxDiff);
}
...
struct OniAndroidLogRedirectCookie
{
    OniAndroidLogRedirectCallback handler;
    void* pCookie;
    XnCallbackHandle handle;
};

void XN_CALLBACK_TYPE oniLogRedirectHandler(const XnLogEntry* pEntry, void* pCookie)
{
    OniAndroidLogRedirectCookie* logRedirectCookie = (OniAndroidLogRedirectCookie*)pCookie;
    (*logRedirectCookie->handler)(pEntry, logRedirectCookie->pCookie);
}

ONI_C_API OniStatus oniRegisterAndroidLogRedirectCallback(OniAndroidLogRedirectCallback handler, void* pCookie, OniCallbackHandle* pHandle)
{
    XnStatus rc = ONI_STATUS_OK;
    g_Context.clearErrorLogger();

    if (*pHandle != NULL)
    {
        // Already registered to something
        g_Context.addToLogger("Can't register same android log redirect listener instance to multiple events");
        return ONI_STATUS_ERROR;
    }

    OniAndroidLogRedirectCookie* logRedirectCookie = XN_NEW(OniAndroidLogRedirectCookie);
    XN_VALIDATE_PTR(logRedirectCookie, ONI_STATUS_ERROR);

    logRedirectCookie->handler = handler;
    logRedirectCookie->pCookie = pCookie;
    *pHandle = (OniCallbackHandle)logRedirectCookie;

    rc = xnLogRegisterAndroidOutputRedirectCallback(oniLogRedirectHandler, logRedirectCookie, &(logRedirectCookie->handle));
    if (rc != XN_STATUS_OK)
    {
        return ONI_STATUS_ERROR;
    }
...
ONI_C_API void oniUnRegisterAndroidLogRedirectCallback(OniCallbackHandle pHandle)
{
    ...
    OniAndroidLogRedirectCookie* logRedirectCookie = (OniAndroidLogRedirectCookie*)pHandle;
    if (logRedirectCookie == NULL)
    {
        return;
    }

    xnLogUnRegisterAndroidOutputRedirectCallback(logRedirectCookie->handle);
    XN_DELETE(logRedirectCookie);
}


ONI_C_API OniStatus oniSetLogAndroidOutputRedirect(OniBool bAndroidOutput)
{
    XnStatus rc = xnLogSetAndroidOutputRedirect((XnBool)bAndroidOutput);
    if (rc != XN_STATUS_OK)
    {
        return ONI_STATUS_ERROR;
    }
...
{
    g_Context.clearErrorLogger();
    return depthStream->pStream->convertC2DCoordinates(colorX, colorY, depthZ, pdepthX, pdepthY);
}

ONI_C_API OniStatus oniCoordinateConverterD2C(OniStreamHandle depthStream, int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY)
{
    g_Context.clearErrorLogger();
    return depthStream->pStream->convertD2CCoordinates(depthX, depthY, depthZ, pColorX, pColorY);
}
#ifdef FREE_SELINUX
ONI_C_API void oniSetUsbParam(int fd, const char* usbpath)
{
    g_Context.SetUsbParam(fd, usbpath);
}
#endif
XN_API_EXPORT_INIT()
