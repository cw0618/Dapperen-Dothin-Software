...
OniStatus VideoStream::start()
{
    if (!m_started)
    {
        ...
        if (m_pSensor->m_startedStreamCount == 0)
        {
			...
			m_errorLogger.Append("This stream requiredFrameSize is %d",requiredFrameSize);
...
void VideoStream::filterSpeckles(void* srcbuf, int newVal, int maxSpeckleSize, int maxDiff){

    int width = m_worldConvertCache.resolutionX, height = m_worldConvertCache.resolutionY, npixels = width*height;
...
OniStatus VideoStream::convertC2DCoordinates(int colorX, int colorY, OniDepthPixel depthZ, int* pDepthX, int* pDepthrY)
{
    if (m_pSensorInfo->sensorType != ONI_SENSOR_DEPTH)
    {
        m_errorLogger.Append("convertColorToDepthCoordinates: Streams are from the wrong sensors (should be DEPTH and COLOR)\n");
        return ONI_STATUS_NOT_SUPPORTED;
    }

    return m_driverHandler.convertPointC2D(m_pSensor->streamHandle(), colorX, colorY, depthZ, pDepthX, pDepthrY);
}

OniStatus VideoStream::convertD2CCoordinates(int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY)
{
    if (m_pSensorInfo->sensorType != ONI_SENSOR_DEPTH)
    {
        m_errorLogger.Append("convertColorToDepthCoordinates: Streams are from the wrong sensors (should be DEPTH and COLOR)\n");
    ...
    return m_driverHandler.convertPointD2C(m_pSensor->streamHandle(), depthX, depthY, depthZ, pColorX, pColorY);
}
