...
DepthUtilsImpl::DepthUtilsImpl() : m_pDepthToShiftTable_QQVGA(NULL), m_pDepthToShiftTable_QVGA(NULL), m_pDepthToShiftTable_VGA(NULL), m_pDepthToShiftTable_SXGA(NULL),
									m_pRegistrationTable_QQVGA(NULL), m_pRegistrationTable_QVGA(NULL), m_pRegistrationTable_VGA(NULL), m_pRegistrationTable_SXGA(NULL), m_bD2SAlloc(false), m_bInitialized(FALSE)
{
}
...
XnStatus DepthUtilsImpl::Initialize(DepthUtilsSensorCalibrationInfo* pBlob)
{
	...
	XN_VALIDATE_ALIGNED_CALLOC(m_pRegistrationTable_SXGA, XnUInt16, 1280 * 1024* 2, XN_DEFAULT_MEM_ALIGN);
	...
	XN_IS_STATUS_OK(nRetVal);
	nRetVal = BuildRegistrationTable(m_pRegistrationTable_SXGA, &m_blob.params1080.registrationInfo_SXGA, &m_pDepthToShiftTable_SXGA, 1280, 1024);
	XN_IS_STATUS_OK(nRetVal);
...
XnStatus DepthUtilsImpl::Free()
{
	...
	if (m_pRegistrationTable_SXGA != NULL)
	{
		xnOSFreeAligned(m_pRegistrationTable_SXGA);
		m_pRegistrationTable_SXGA = NULL;
	}
	...
	if (m_bD2SAlloc)
	{
		...
		if (m_pDepthToShiftTable_SXGA != NULL)
		{
			xnOSFreeAligned(m_pDepthToShiftTable_SXGA);
			m_pDepthToShiftTable_SXGA = NULL;
		}
...
XnStatus DepthUtilsImpl::SetDepthConfiguration(int xres, int yres, OniPixelFormat /*format*/, bool isMirrored)
{
	...
	else if ((xres == 800 && yres == 1280) || (xres == 400 && yres == 640) || (xres == 1280 && yres == 720)
		|| (xres == 480 && yres == 640) || (xres == 1080 && yres == 1280) || (xres == 960 && yres == 1280)
		|| (xres == 540 && yres == 640) || (xres == 720 && yres == 960) || (xres == 1024 && yres == 1280)
		|| (xres == 800 && yres == 448) || (xres == 864 && yres == 480) || (xres == 1328 && yres == 1120)
		|| (xres == 1280 && yres == 1120) || (xres == 664 && yres == 560) || (xres == 640 && yres == 560)
		|| (xres == 960 && yres == 1120) || (xres == 480 && yres == 560) || (xres == 640 && yres == 960)
        || (xres == 640 && yres == 600))
	{
	}
	else
