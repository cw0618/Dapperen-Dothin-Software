...
DeviceParameter g_PhaseCapturing;
...
void captureInit()
{
    ...
    // Phase Formats
    nIndex = 0;
    g_PhaseCapturing.pValues[nIndex] = STREAM_CAPTURE_LOSSLESS;
    g_PhaseCapturing.pValueToName[nIndex] = "Lossless";
    nIndex++;
    g_PhaseCapturing.pValues[nIndex] = STREAM_DONT_CAPTURE;
    g_PhaseCapturing.pValueToName[nIndex] = "Don't Capture";
    nIndex++;
    g_PhaseCapturing.nValuesCount = nIndex;
    ...
    g_Capture.streams[CAPTURE_PHASE_STREAM].captureType = STREAM_CAPTURE_LOSSLESS;
    g_Capture.streams[CAPTURE_PHASE_STREAM].name = "Phase";
    g_Capture.streams[CAPTURE_PHASE_STREAM].getFrameFunc = getPhaseFrame;
    g_Capture.streams[CAPTURE_PHASE_STREAM].getStream = getPhaseStream;
    g_Capture.streams[CAPTURE_PHASE_STREAM].isStreamOn = isPhaseOn;
}
...
void captureSetPhaseFormat(int format)
{
    g_Capture.streams[CAPTURE_PHASE_STREAM].captureType = (StreamCaptureType)format;
}
...
void getColorFileName(int num, char* csName, const openni::VideoFrameRef* pFrame)
{
    if (!pFrame->isValid())
        sprintf(csName, "%s/color_%d.raw", CAPTURED_FRAMES_DIR_NAME, num);
    else
    {
        sprintf(csName, "%s/color_%dx%d_%lld_%d_%d.raw",
            CAPTURED_FRAMES_DIR_NAME,
            pFrame->getWidth(),
            pFrame->getHeight(),
            (long long)pFrame->getTimestamp(),
...
void getDepthFileName(int num, char* csName, const openni::VideoFrameRef* pFrame)
{
    if (!pFrame->isValid())
        sprintf(csName, "%s/depth_%d.raw", CAPTURED_FRAMES_DIR_NAME, num);
    else
    {
        sprintf(csName, "%s/depth_%dx%d_%lld_%d_%d.raw",
            CAPTURED_FRAMES_DIR_NAME,
            pFrame->getWidth(),
            pFrame->getHeight(),
            (long long)pFrame->getTimestamp(),
...
void getIRFileName(int num, char* csName, const openni::VideoFrameRef* pFrame)
{
    if (!pFrame->isValid())
        sprintf(csName, "%s/ir_%d.raw", CAPTURED_FRAMES_DIR_NAME, num);
    else
    {
        sprintf(csName, "%s/ir_%dx%d_%lld_%d_%d.raw",
            CAPTURED_FRAMES_DIR_NAME,
            pFrame->getWidth(),
            pFrame->getHeight(),
            (long long)pFrame->getTimestamp(),
...
void getPhaseFileName(int num, char* csName, const openni::VideoFrameRef* pFrame)
{
    if (!pFrame->isValid())
        sprintf(csName, "%s/phase_%d.raw", CAPTURED_FRAMES_DIR_NAME, num);
    else
    {
        sprintf(csName, "%s/phase_%dx%d_%lld_%d_%d.raw",
            CAPTURED_FRAMES_DIR_NAME,
            pFrame->getWidth(),
            pFrame->getHeight(),
            (long long)pFrame->getTimestamp(),
...
void getAIFileName(int num, char* csName, const openni::VideoFrameRef* pFrame)
{
    if (!pFrame->isValid())
        sprintf(csName, "%s/ai_%d.raw", CAPTURED_FRAMES_DIR_NAME, num);
    else
    {
        sprintf(csName, "%s/ai_%dx%d_%lld_%d_%d.raw",
            CAPTURED_FRAMES_DIR_NAME,
            pFrame->getWidth(),
            pFrame->getHeight(),
            (long long)pFrame->getTimestamp(),
...
int findUniqueFileName()
{
	...
	XnBool bExist = FALSE;
	XnStatus nRetVal = XN_STATUS_OK;
    XnChar csColorFileName[XN_FILE_MAX_PATH] = { 0 };
    XnChar csDepthFileName[XN_FILE_MAX_PATH] = { 0 };
    XnChar csIRFileName[XN_FILE_MAX_PATH] = { 0 };
    XnChar csPhaseFileName[XN_FILE_MAX_PATH] = { 0 };
    XnChar csAIFileName[XN_FILE_MAX_PATH] = { 0 };

	for (;;)
	{
		// check color
		getColorFileName(num, csColorFileName, &getColorFrame());
		...
		{
			// check depth
			getDepthFileName(num, csDepthFileName, &getDepthFrame());
		...
		{
			// check IR
			getIRFileName(num, csIRFileName, &getIRFrame());
        ...
        {
            /// Check phase.
            getPhaseFileName(num, csPhaseFileName, &getPhaseFrame());
            nRetVal = xnOSDoesFileExist(csPhaseFileName, &bExist);
            if (nRetVal != XN_STATUS_OK || !bExist)
                break;
        }
        ...
        {
            /// Check AI.
            getAIFileName(num, csAIFileName, &getAIFrame());
            nRetVal = xnOSDoesFileExist(csAIFileName, &bExist);
            if (nRetVal != XN_STATUS_OK || !bExist)
                break;
        }
...
void captureSingleFrame(int)
{
	...
	{
        getColorFileName(num, csColorFileName, &colorFrame);
	...
	{
        getDepthFileName(num, csDepthFileName, &depthFrame);
	...
	if (irFrame.isValid())
	{
        getIRFileName(num, csIRFileName, &irFrame);
		xnOSSaveFile(csIRFileName, irFrame.getData(), irFrame.getDataSize());
	}

    openni::VideoFrameRef& phaseFrame = getPhaseFrame();
    if (phaseFrame.isValid())
    {
        getPhaseFileName(num, csPhaseFileName, &phaseFrame);
        xnOSSaveFile(csPhaseFileName, phaseFrame.getData(), phaseFrame.getDataSize());
    }

    openni::AIFrameRef& aiFrame = getAIFrame();
    if (aiFrame.isValid())
    {
        getAIFileName(num, csAIFileName, &aiFrame);
        if (aiFrame.getBodyMask().isValid())
        {
            std::string fileName(csAIFileName);
            fileName.insert(fileName.find_first_of("_") + 1, "body_mask_");
            xnOSSaveFile(fileName.c_str(), aiFrame.getBodyMask().getData(), aiFrame.getBodyMask().getDataSize());
        }

        if (aiFrame.getFloorInfo().isValid())
        {
            std::string fileName(csAIFileName);
            fileName.insert(fileName.find_first_of("_") + 1, "floor_mask_");
            xnOSSaveFile(fileName.c_str(), aiFrame.getFloorInfo().getData(), aiFrame.getFloorInfo().getDataSize());
        }
    }
...
const char* captureGetPhaseFormatName()
{
    return getCaptureTypeName(g_Capture.streams[CAPTURE_PHASE_STREAM].captureType);
}
