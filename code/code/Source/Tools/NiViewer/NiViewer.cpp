...
void createKeyboardMap()
{
    startKeyboardMap();
    {
        ...
        {
            registerKey('1', getPresetName(1), setPreset, 2);
            registerKey('2', getPresetName(2), setPreset, 3);
            registerKey('3', getPresetName(3), setPreset, 4);
            registerKey('4', getPresetName(4), setPreset, 5);
            registerKey('5', getPresetName(5), setPreset, 6);
            ...
            registerSpecialKey(GLUT_KEY_RIGHT, "Up MaxSpeckleSize", SpeckleFilter, GLUT_KEY_RIGHT);
            registerSpecialKey(GLUT_KEY_LEFT, "Down MaxSpeckleSize", SpeckleFilter, GLUT_KEY_LEFT);
            registerSpecialKey(GLUT_KEY_UP, "Up MaxDiff", SpeckleFilter, GLUT_KEY_UP);
            registerSpecialKey(GLUT_KEY_DOWN, "Down MaxDiff", SpeckleFilter, GLUT_KEY_DOWN);
        ...
        startKeyboardGroup(KEYBOARD_GROUP_DEVICE);
        {
            ...
            registerKey('w', "swtich IR", switchIRCamera, 0);  //left ir
            registerKey('b', "swtich IR", switchIRCamera, 1);  //right ir
            registerKey('a', "Toggle AE for color", toggleImageAutoExposure, 0);
            registerKey('A', "Toggle AE for depth", toggleDepthAutoExposure, 0);
            ...
            registerSpecialKey(GLUT_KEY_F5, "IR Flood on/off", toggleFloodState, 0);
        }
        endKeyboardGroup();
    ...
    startMenu();
    {
        ...
        startSubMenu("Device");
        {
            startSubMenu("Streams");
            {
                    ...
                    startSubMenu("Depth");
                    {
                        ...
                        startSubMenu("Video Mode");
                        {
                            ...
                            for (int i = 0; i < supportedModes.getSize(); ++i)
                            {
                                ...
                                char name[50];
                                XnUInt32 dummy;
                                xnOSStrFormat(name, 50, &dummy, "%d x %d @ %d (%s)",
                                    pSupportedMode->getResolutionX(),
                                    pSupportedMode->getResolutionY(),
                                    pSupportedMode->getFps(),
                                    getFormatName(pSupportedMode->getPixelFormat()));
                ...
                // Phase stream
                /*pSensorInfo = getPhaseSensorInfo();
                if (NULL != pSensorInfo)
                {
                    startSubMenu("Phase");
                    {
                        createMenuEntry("On/Off", togglePhaseState, 0);
                        startSubMenu("Video Mode");
                        {
                            const openni::Array<openni::VideoMode>& supportedModes = pSensorInfo->getSupportedVideoModes();
                            for (int i = 0; i < supportedModes.getSize(); ++i)
                            {
                                ...
                                xnOSStrFormat(name, 50, &dummy, "%d x %d @ %d (%s)",
                                    ...
                                    pSupportedMode->getFps(),
                                    getFormatName(pSupportedMode->getPixelFormat()));
                                createMenuEntry(name, setPhaseVideoMode, i);
                            }
                        }
                        endSubMenu();
                        createMenuEntry("Mirror", togglePhaseMirror, 0);
                        createMenuEntry("Reset Cropping", resetPhaseCropping, 0);
                    }
                    endSubMenu();
                }*/

                // AI stream.
                pSensorInfo = getAISensorInfo();
                if (NULL != pSensorInfo)
                {
                    startSubMenu("AI");
                    {
                        createMenuEntry("On/Off", toggleAIState, 0);
                        startSubMenu("Video Mode");
                        {
                            const openni::Array<openni::VideoMode>& supportedModes = pSensorInfo->getSupportedVideoModes();
                            for (int i = 0; i < supportedModes.getSize(); ++i)
                            {
                                ...
                                xnOSStrFormat(name, 50, &dummy, "%d x %d @ %d (%s)",
                                    ...
                                    pSupportedMode->getFps(),
                                    getFormatName(pSupportedMode->getPixelFormat()));
                                createMenuEntry(name, setAIVideoMode, i);
                            }
                        }
                        endSubMenu();
                    }
            ...
            startSubMenu("Frequency Mode");
            {
                for (int i = 0; i < g_frequencyMode.nValuesCount; ++i)
                {
                    unsigned int nValue = g_frequencyMode.pValues[i];
                    createMenuEntry(g_frequencyMode.pValueToName[nValue], changeFrequencyMode, nValue);
                }
            }
            endSubMenu();
        ...
        startSubMenu("Capture");
        {
            ...
            endSubMenu();
            startSubMenu("Phase Capturing");
            {
                for (int i = 0; i < g_PhaseCapturing.nValuesCount; ++i)
                {
                    unsigned int nValue = g_PhaseCapturing.pValues[i];
                    createMenuEntry(g_PhaseCapturing.pValueToName[i], captureSetPhaseFormat, nValue);
                }
            }
            endSubMenu();
...
int main(int argc, char **argv)
{
	...
	config.openPhase = SENSOR_TRY;
    config.openAI = SENSOR_OFF;
#if 1
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (strcmp(argv[i], "-phase=on") == 0)
        {
            config.openPhase = SENSOR_ON;
        }
        else if (strcmp(argv[i], "-phase=off") == 0)
        {
            config.openPhase = SENSOR_OFF;
        }
        else if (strcmp(argv[i], "-phase=try") == 0)
        {
            config.openPhase = SENSOR_TRY;
        }
        else if (strcmp(argv[i], "-ai=on") == 0)
        {
            config.openAI = SENSOR_ON;
        }
        else if (strcmp(argv[i], "-ai=off") == 0)
        {
            config.openAI = SENSOR_OFF;
        }
        else if (strcmp(argv[i], "-ai=try") == 0)
        {
            config.openAI = SENSOR_TRY;
        }
        else if (uri == NULL)
        {
            uri = argv[i];
...
#endif
	//uri = "D:\\orbbecproject\\2020\\OpenniSDKMaster\\openni2.3\\Bin\\Win32-Release\\config\\testoni.oni";
    // Xiron Init
    ...
    glutPositionWindow((cx - wid) / 2, (cy - hei) / 2);
