...
typedef struct MouseInput
{
	...
	CursorMovedPtr pCursorDownEvent;
} MouseInput;

MouseInput g_MouseInput = {SELECTION_NONE, {0,0}, {0,0}, NULL, NULL, NULL};
...
void mouseInputButton(int button, int state, int x, int y)
{
	if (button == GLUT_LEFT_BUTTON)
	{
		if (state == GLUT_DOWN)
		{
			...
			if(g_MouseInput.pCursorDownEvent != NULL){
				g_MouseInput.pCursorDownEvent(g_MouseInput.StartSelection);
			}
		}
...
void mouseInputRegisterForCursorDownEvent(CursorMovedPtr pFunc)
{
	g_MouseInput.pCursorDownEvent = pFunc;
}
