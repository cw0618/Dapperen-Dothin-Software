...
#define XN_MASK_NIVIEWER_DRAW "NIVIEWER_DRAW"
...
DrawConfigPreset g_Presets[PRESET_COUNT] =
{
    ...
    { "Side by Side Normal",                { { DEPTH_NORMAL,             1 },          { COLOR_NORMAL },       SIDE_BY_SIDE } },
    { "Side by Side HISTOGRAM",             { { LINEAR_HISTOGRAM,         1 },          { COLOR_NORMAL },       SIDE_BY_SIDE } },
...
void drawCursorDown(IntPair location)
{
    if (isPointInRect(location, &g_DrawConfig.DepthLocation)){
        g_DrawUserInput.downInDepth = location;
    }
}
...
void drawInit()
{
    ...
    g_DepthDrawColoring[DEPTH_NORMAL] = "Normal";
    ...
    mouseInputRegisterForCursorDownEvent(drawCursorDown);
}
...
void calculateHistogram()
{
    ...
    openni::VideoFrameRef* pFrame = NULL;
    if (isDepthOn())
    {
        pFrame = &getDepthFrame();
        pStream = &getDepthStream();
    }
    else if (isPhaseOn())
    {
        pFrame = &getPhaseFrame();
        pStream = &getPhaseStream();
    }
    else
        return;

    if (!pStream->isValid() || !pFrame->isValid())
        return;

    int maxDepthValue = MAX_DEPTH_VALUE/*getDepthStream().getMaxPixelValue() + 1*/;
    ...
    const openni::DepthPixel* pDepth = (const openni::DepthPixel*)pFrame->getData();
    const openni::DepthPixel* pDepthEnd = pDepth + (pFrame->getDataSize() / sizeof(openni::DepthPixel));
...
void dumpMetadata(const openni::Metadata& meta)
{
    static uint32_t num = 0;
    static uint32_t counter = 0;
    if (num++ < 30 && 0 == counter % 3000)
    {
        printf("---------------------------------- Meta ---------------------------------\n");
        ...
        printf("        Frequency[0]    : %d\n", meta.getLowFreq());
        printf("        Frequency[1]    : %d\n", meta.getHighFreq());
        printf("        Intergration[0] : %d\n", meta.getLowIntegration());
        printf("        Intergration[1] : %d\n", meta.getHighIntegration());
        printf("        Duty Cycle[0]   : %f\n", meta.getLowDutyCycle());
        printf("        Duty Cycle[1]   : %f\n", meta.getHighDutyCycle());
        printf("        RX Temperature  : %f\n", meta.getTemperRX());
        printf("        TX Temperature  : %f\n", meta.getTemperTX());
        printf("-------------------------------------------------------------------------\n");
    }
    else
    {
...
void drawColor(IntRect* pLocation, IntPair* pPointer, int pointerRed, int pointerGreen, int pointerBlue)
{
    ...
    if (!isColorOn() && !isIROn())
    {
        drawClosedStream(pLocation, "IR | Color");
        return;
    }
    ...
    dumpMetadata(colorMD.getMetadata());
    ...
    // create IR histogram
    // gray16bit and rgb888 use the same method
    /*
        ...
        else
        {
            ...
            for (XnUInt16 nX = 0; nX < width; nX++, pTexture += 4)
            {
                ...
                case openni::PIXEL_FORMAT_GRAY16:
                    p16 = (XnUInt16*)pColor;
                    pTexture[0] = pTexture[1] = pTexture[2] = (XnUInt8)((*p16) >> 2 /*grayscale16Factor*/);
                    pColor += 2;
                    break;
                default:
                    assert(0);
...
void drawDepth(IntRect* pLocation, IntPair* pPointer)
{
    ...
    {
        if (!isDepthOn() && !isPhaseOn())
        {
            drawClosedStream(pLocation, "Phase | Depth");
            return;
        }

        openni::VideoFrameRef* pDepthMD = NULL;
        if (isDepthOn())
            pDepthMD = &getDepthFrame();
        else if (isPhaseOn())
            pDepthMD = &getPhaseFrame();

        if (NULL == pDepthMD || !pDepthMD->isValid())
            return;
#if 0
        glRasterPos2i(10, g_DrawConfig.DepthLocation.uBottom);
        char csMessage[512];
        XnUInt32 nWritten;
        xnOSStrFormat(csMessage, sizeof(csMessage), &nWritten, "MaxSpeckleSize: %d, MaxDiff: %d ", g_maxSpeckleSize, g_maxDiff);
        glPrintString(GLUT_BITMAP_HELVETICA_18, csMessage);

        getDepthStream().filterSpeckles(pDepthMD->_getFrame()->data, 0, g_maxSpeckleSize, g_maxDiff);
#endif
        ...
        dumpMetadata(pDepthMD->getMetadata());
        ...
        for (XnUInt16 nY = originY; nY < height + originY; nY++)
        {
            ...
            for (XnUInt16 nX = 0; nX < width; nX++, pDepth++, pTexture += 4)
            {
                ...
                {
                case DEPTH_NORMAL:
                    nRed = nGreen = nBlue = g_pDepthHist[*pDepth] * 255;
                    break;
...
void drawAI(IntRect* /*pLocation*/, IntPair* /*pPointer*/)
{
    if (NULL == getAISensorInfo() || !isAIOn())
        return;

    openni::AIFrameRef& frame = getAIFrame();
    if (!frame.isValid())
    {
        displayError("No AI frame detected...");
        return;
    }

    displayMessage("Receive AI Frame: total body number = %d, pixel format = 0x%x, timestamp = %lld",
        frame.getBodySize(),
        frame.getVideoMode().getPixelFormat(),
        frame.getTimestamp());

    printf("\n[%s] ========================================== AI Frame %d =========================================\n", XN_MASK_NIVIEWER_DRAW, frame.getFrameIndex());

    // Joint & Body Shape.
    if (frame.getBodySize() > 0)
    {
        for (int i = 0; i < frame.getBodySize(); ++i)
        {
            openni::Body body = frame.getBodyByIndex(i);
            if (openni::AI_STATUS_TRACKING == body.getJointStatus())
            {
                printf("\n\n--------------------------------- Joint ----------------------------------\n");
                printf("Body Joint: id = %d, format = 0x%x, color resolution = %dx%d, timestamp = %lld\n",
                    body.getID(),
                    body.getJointFormat(),
                    body.getJointWidth(),
                    body.getJointHeight(),
                ...
                {
                    openni::Joint joint = body.getJointByIndex(j);
                    printf("Joint: type = %d, score = %f, world coordinate = { %f, %f, %f }\n",
                        joint.getType(),
                        joint.getConfidence(),
                        joint.getWorldCoordinate().x,
                        joint.getWorldCoordinate().y,
                        joint.getWorldCoordinate().z);
                }
                printf("\n--------------------------------------------------------------------------\n");
            }

            if (openni::AI_STATUS_TRACKING == body.getBodyShapeStatus())
            {
                printf("\n\n------------------------------- Body Shape -------------------------------\n");
                printf("Body Shape: id = %d, height = %f(mm)\n", body.getID(), body.getBodyShapeHeight());
                printf("\n--------------------------------------------------------------------------\n");
            }
        }
    }
    ...
    // Body Mask.
    openni::BodyMask bodyMask = frame.getBodyMask();
    if (bodyMask.isValid())
    {
        printf("\n\n------------------------------- Body Mask --------------------------------\n");
        printf("    width     = %d\n", bodyMask.getWidth());
        printf("    height    = %d\n", bodyMask.getHeight());
        printf("    data size = %d\n", bodyMask.getDataSize());
        printf("    data addr = %p\n", bodyMask.getData());
        printf("----------------------------------------------------------------------------\n");
    }

    // Floor Info.
    openni::FloorInfo floorInfo = frame.getFloorInfo();
    if (floorInfo.isValid())
    {
        printf("\n\n------------------------------- Floor Info -------------------------------\n");
        printf("    width        = %d\n", floorInfo.getWidth());
        printf("    height       = %d\n", floorInfo.getHeight());
        printf("    data size    = %d\n", floorInfo.getDataSize());
        printf("    data addr    = %p\n", floorInfo.getData());
        printf("    plane center = { %f, %f, %f }\n",
            floorInfo.getPlaneCenter().x,
            floorInfo.getPlaneCenter().y,
            floorInfo.getPlaneCenter().z);
        printf("    plane normal = { %f, %f, %f }\n",
            floorInfo.getPlaneNormal().x,
            floorInfo.getPlaneNormal().y,
            floorInfo.getPlaneNormal().z);
        printf("----------------------------------------------------------------------------\n");
    }

    // Phase
    openni::TOFFrame phase = frame.getFrameSet().getTOFFrame(openni::FRAME_TYPE_PHASE);
    if (phase.isValid())
    {
        printf("\n\n------------------------------- Phase -------------------------------\n");
        printf("        plane num:    %d\n", phase.getPlaneNum());
        ...
        static int cnt = 0;
        char fileName[128] = { 0 };
        for (int i = 0; i < phase.getPlaneNum(); ++i)
        {
            dumpMetadata(phase.getMetadata(i));
            if (cnt < phase.getPlaneNum())
            {
                ++cnt;
                sprintf(fileName, "phase-%dx%d-%d.raw", phase.getWidth(), phase.getHeight(), i);
                FILE *fp = fopen(fileName, "wb");
                fwrite(phase.getPlane(i), phase.getPlaneSize(), 1, fp);
            }
        }
        printf("-----------------------------------------------------------------------\n");
    }
    ...
    // Depth
    openni::TOFFrame depth = frame.getFrameSet().getTOFFrame(openni::FRAME_TYPE_DEPTH);
    if (depth.isValid())
    {
        printf("\n\n------------------------------- Depth -------------------------------\n");
        printf("        plane num:    %d\n", depth.getPlaneNum());
        ...
        static int cnt = 0;
        char fileName[128] = { 0 };
        for (int i = 0; i < depth.getPlaneNum(); ++i)
        {
            dumpMetadata(depth.getMetadata(i));
            if (cnt < depth.getPlaneNum())
            {
                ++cnt;
                sprintf(fileName, "depth-%dx%d-%d.raw", depth.getWidth(), depth.getHeight(), i);
                FILE *fp = fopen(fileName, "wb");
                fwrite(depth.getPlane(i), depth.getPlaneSize(), 1, fp);
            }
        }
        printf("-----------------------------------------------------------------------\n");
    }
    ...
    // IR
    openni::TOFFrame ir = frame.getFrameSet().getTOFFrame(openni::FRAME_TYPE_IR);
    if (ir.isValid())
    {
        printf("\n\n------------------------------- IR -------------------------------\n");
        printf("        plane num:    %d\n", ir.getPlaneNum());
        ...
        static int cnt = 0;
        char fileName[128] = { 0 };
        for (int i = 0; i < ir.getPlaneNum(); ++i)
        {
            dumpMetadata(ir.getMetadata(i));
            if (cnt < ir.getPlaneNum())
            {
                ++cnt;
                sprintf(fileName, "ir-%dx%d-%d.raw", ir.getWidth(), ir.getHeight(), i);
                FILE *fp = fopen(fileName, "wb");
                fwrite(ir.getPlane(i), ir.getPlaneSize(), 1, fp);
            }
        }
        printf("-----------------------------------------------------------------------\n");
    }

    printf("\n[%s] =============================================================================================\n\n", XN_MASK_NIVIEWER_DRAW);
    //frame.release();
}
...
void drawPointerMode(IntPair* pPointer)
{
    ...
    openni::VideoFrameRef* pPhaseMD = &getPhaseFrame();
    if (pPhaseMD->isValid())
    {
        if (buf[0] != '\0')
        {
        ...
        xnOSStrFormat(buf + strlen(buf), (XnUInt32)(sizeof(buf) - strlen(buf)), &chars, "%s - Frame %4u, Timestamp %.3f", "Phase", pPhaseMD->getFrameIndex(), (double)pPhaseMD->getTimestamp() / dTimestampDivider);
    }
    ...
    if (pPointer != NULL)
    {
        ...
        glRasterPos2i(10, nYLocation - 10);
        glPrintString(GLUT_BITMAP_HELVETICA_18, buf);
        nYLocation -= 26;
...
{
    if (!(isIROn() || isDepthOn() || isColorOn() || isPhaseOn() || isAIOn()))
        return;
    ...
    xnOSStrFormat(csMessage, sizeof(csMessage), &nWritten,
        "Image registration is %s. Capture Formats - Depth: %s | Image: %s | IR: %s | Phase: %s",
...
#if 1
void drawUserdrag(IntPair *depthPoint)
{
    if (depthPoint == NULL){
        return;
    }
    ...
    if (g_DrawUserInput.State == SELECTION_ACTIVE)
    {
        ...
        openni::VideoStream& depthStream = getDepthStream();
        openni::VideoFrameRef* pDepthMD = &getDepthFrame();
        if (!pDepthMD->isValid())
            return;

#if 1
        int downX = (double)(g_DrawUserInput.downInDepth.X - g_DrawConfig.DepthLocation.uLeft)
            / (g_DrawConfig.DepthLocation.uRight - g_DrawConfig.DepthLocation.uLeft + 1) * pDepthMD->getVideoMode().getResolutionX();
        int  downY = (double)(g_DrawUserInput.downInDepth.Y - g_DrawConfig.DepthLocation.uBottom)
            / (g_DrawConfig.DepthLocation.uTop - g_DrawConfig.DepthLocation.uBottom + 1) * pDepthMD->getVideoMode().getResolutionY();
#endif

        const openni::DepthPixel* pDepth = (const openni::DepthPixel*)getDepthFrame().getData();


        int index2 = pDepthMD->getVideoMode().getResolutionX()*downY + downX;
        int index = pDepthMD->getVideoMode().getResolutionX()*depthPoint->Y + depthPoint->X;
        if (index2 < 0 || index < 0){
            return;
        }
        openni::DepthPixel downPixel = pDepth[index2];
        World3D start;
        openni::CoordinateConverter::convertDepthToWorld(depthStream, downX, downY, downPixel,
            &start.X, &start.Y, &start.Z);

        World3D end;
        openni::DepthPixel cursorPixel = pDepth[index];
        openni::CoordinateConverter::convertDepthToWorld(depthStream, depthPoint->X, depthPoint->Y, cursorPixel,
            &end.X, &end.Y, &end.Z);


        double dragX = abs(end.X - start.X);
        double dragY = abs(end.Y - start.Y);
        double dragZ = abs(end.Z - start.Z);

        double proX = sqrt(dragX*dragX + dragY*dragY);
        double distance = sqrt(dragZ*dragZ + proX*proX);

        char buf[100];
        memset(buf, 0, sizeof(buf));
        XnUInt dummy;
        if (downPixel == 0 || cursorPixel == 0)
		{
            xnOSStrFormat(buf, sizeof(buf), &dummy, "start point or end point invalid");
        }
        else
		{
            xnOSStrFormat(buf, sizeof(buf), &dummy, "2D distance = %lf, 3D distance = %lf mm", proX, distance);
        }

        glPointSize(3);
        glBegin(GL_POINTS);
        glColor3f(1, 0, 0);
        glVertex2i(g_DrawUserInput.downInDepth.X, g_DrawUserInput.downInDepth.Y);
        glVertex2i(g_DrawUserInput.Cursor.X, g_DrawUserInput.Cursor.Y);
        glEnd();
        ...
        glColor4f(1, 0, 0, 0);
        glRasterPos2i(g_DrawUserInput.Cursor.X, g_DrawUserInput.Cursor.Y);
        glPrintString(GLUT_BITMAP_HELVETICA_18, buf);
    }
}
#endif
...
void drawFrame()
{
    ...
    if (NULL != pDepthMD && pDepthMD->isValid())
    {
        int resY = pDepthMD->getVideoMode().getResolutionY();
        if (resY == 400)
        {
            resY = 480;
        }

        TextureMapInit(&g_texDepth, pDepthMD->getVideoMode().getResolutionX(), resY, 4, pDepthMD->getWidth(), resY);
        fixLocation(&g_DrawConfig.DepthLocation, pDepthMD->getVideoMode().getResolutionX(), resY);
    }
    ...
    if (NULL != pImageMD && pImageMD->isValid())
    {
