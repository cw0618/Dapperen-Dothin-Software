...
XnStatus XnMx6xModulesHelper::LoadModules()
{
	...
	{
		XnChar* moduleName = acsFileList[i];
		
		if (strcmp(moduleName, "rgb.dll") == 0)
		{
		...
		XN_VALIDATE_STR_APPEND(rscModulePath, moduleName, XN_FILE_MAX_PATH, nRetVal);

		XnChar desModulePath[XN_FILE_MAX_PATH];
		nRetVal = CreateModuleFilesStr(moduleName, desModulePath);
...
XnStatus XnMx6xModulesHelper::CommandSetProperty(XnInt propertyId, void* data, XnInt dataSize)
{
	...
	XnInt ret = p_mx6x_handle_->set_property(obc_Cmd.command, &cmd_t);
	if (ret != XN_STATUS_OK)
	{
