...
XnStatus XnPlecoTofSensorCalc::UpdateCalcConfig()
{
    ...
    if (m_phasePixelNum != m_phaseWidth * m_phaseHeight * m_phaseOutputCount)
    {
		m_phasePixelNum = m_phaseWidth * m_phaseHeight * m_phaseOutputCount;
        delete[] m_phaseBuf;
        m_phaseBuf=nullptr;
        m_phaseBuf = new XnInt16[m_phasePixelNum];
    }
...
int XnPlecoTofSensorCalc::getWidth(){
	return kVGADepthWidth;
}
int XnPlecoTofSensorCalc::getHeight(){
