...
XnTofSensor(mx6xModule)
{
	...
	m_phaseInputCount = kDualFreqPhaseInputNum;
	m_phaseOutputCount = kDualFreqPhaseOutputNum;
...
XnStatus XnPlecoTofSensor::CalcSensorProperty()
{
	...
	case TOF_SINGLE_FREQUENCY_MODE:
		m_phaseWidth = kOriginalPhaseWidth / m_binningFactor;
		m_phaseHeight = kOriginalPhaseHeight / m_binningFactor;
		m_phaseInputCount = kSingleFreqPhaseInputNum;
		m_phaseOutputCount = kSingleFreqPhaseOutputNum;
		m_grabWidth = m_phaseWidth;
	...
	xnLogInfo(XN_MASK_PLECO_TOF_SENSOR, "CalcSensorProperty, m_freqMode %d, m_binningFactor %d, grab_w %d, grab_h %d", m_freqMode, m_binningFactor, m_grabWidth, m_grabHeight);
...
XnStatus XnPlecoTofSensor::GetPhaseFrame(const XnUChar* dothinInputBuf, const XnUInt32 dothinInputBufSize, XnUInt16 *frameData, XnUInt32 *frameDataSize)
{
	...
	//xnOSSaveFile("D:\\orbbecproject\\2020\\OpenniSDK\\openni2.3\\Bin\\Win32-Debug\\plecoFrameData.raw", frameDataAddr, outputImageSize);

	return  XN_STATUS_OK;
}

XnStatus XnPlecoTofSensor::GetIRFrame(const XnUChar *dothinInputBuf, const XnUInt32 dothinInputBufSize, XnUInt16 *frameData, XnUInt32 *frameDataSize)
{
	if (nullptr == dothinInputBuf || nullptr == frameData
		|| nullptr == m_pFrameGroup || nullptr == m_pFrameGroup->frames
		|| nullptr == m_pCompressedFrameGroup || nullptr == m_pCompressedFrameGroup->frames)
	...
	XnUInt16* frameDataIR = (XnUInt16*)((XnUInt8*)frameData + kVGADepthWidth*kBytePerDepthPixel*m_extraLine);
	ret = CalcIRFramePleco(m_pFrameGroup, frameDataIR, kVGADepthWidth, kVGADepthHeight);
	//xnOSSaveFile(".\\irFrameData.raw", frameData + kVGADepthWidth*kBytePerDepthPixel*m_extraLine, kVGADepthWidth * kVGADepthHeight * kBytePerDepthPixel);
	return  XN_STATUS_OK;
}

XnStatus XnPlecoTofSensor::GetDepthFrame(const XnUChar *dothinInputBuf, const XnUInt32 dothinInputBufSize, XnUInt16 *frameData, XnUInt32 *frameDataSize)
{
	if (nullptr == dothinInputBuf || nullptr == frameData
		|| nullptr == m_pFrameGroup || nullptr == m_pFrameGroup->frames
		|| nullptr == m_pCompressedFrameGroup || nullptr == m_pCompressedFrameGroup->frames)
	...
	//xnOSSaveFile("D:\\orbbecproject\\2020\\OpenniSDK\\openni2.3\\Bin\\Win32-Debug\\PhaseFrameData.raw", m_phaseBuf, kOriginalPhaseWidth * kOriginalPhaseHeight * kBytePerDepthPixel*6);
	...
	XnUInt16* frameDataDepth = (XnUInt16*)((XnUInt8*)frameData + kVGADepthWidth*sizeof(XnUInt16)*m_extraLine);
...
XnStatus XnPlecoTofSensor::GetAIFrame(const XnUChar* dothinInputBuf, const XnUInt32 dothinInputBufSize, OniAIFrame* pAiFrame)
{
...
XnStatus XnPlecoTofSensor::setFrameResolution(int width, int height)
{
