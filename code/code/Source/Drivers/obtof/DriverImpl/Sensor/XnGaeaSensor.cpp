#include "XnGaeaSensor.h"
#include "OniCTypes.h"
#include "OpenNI.h"
#include"tofinfo.h"

XnGaeaSensor::XnGaeaSensor(mx6x_module_t *mx6xModule) :
...
XnGaeaSensor::~XnGaeaSensor()
{
	m_configFilePathAndName = nullptr;
	m_calibFilePathAndName = nullptr;
}

XnStatus XnGaeaSensor::Init()
{
	return XN_STATUS_OK;
}

XnStatus XnGaeaSensor::CalcSensorProperty()
{
	switch (m_freqMode)
	{
	case TOF_SINGLE_FREQUENCY_MODE:
		//m_phaseWidth = m_windowWidth / m_binningFactor;
		//m_phaseHeight = m_windowHeight / m_binningFactor;
		//m_phaseWidth = kOriginalPhaseWidth;
		//m_phaseHeight = kOriginalPhaseHeight;
		
		if (m_subsamp == 0){
			m_phaseWidth = m_windowWidth / m_binningFactor;
		}
		else{
			m_phaseWidth = m_windowWidth / (m_subsamp + 1);
		}
		if (m_subsampv == 0){
			m_phaseHeight = m_windowHeight / m_binningFactor;
		}
		else{
			m_phaseHeight = m_windowHeight / (m_subsampv + 1);
		}
	...
	xnLogInfo(XN_MASK_GAEA_TOF_SENSOR, "CalcSensorProperty, m_freqMode %d, m_binningFactor %d, grab_w %d, grab_h %d ,m_phaseWidth %d,m_phaseHeight %d", m_freqMode, m_binningFactor, m_grabWidth, m_grabHeight, m_phaseWidth, m_phaseHeight);

	return XN_STATUS_OK;
}

XnStatus XnGaeaSensor::GetDutyCyclePercentFormat(uint32_t freq, uint32_t ducy_cle, XnInt sensor_id, float *pduty_cycle_percent_format)
{
...
XnStatus XnGaeaSensor::GetIllumDutyCycleList(XnUInt8 mod_freq, XnFloat *duty_cycle_list)
{
...
XnStatus XnGaeaSensor::UnpackCompressedIRFrameGroup(ObFrameGroup *pCompressedFrameGroup, ObFrameGroup *pFramegroup, int phaseIndex)
{
	if (nullptr == pCompressedFrameGroup || nullptr == pFramegroup)
	{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "pFrameGroup is nullptr");
	...
	if (nullptr == pCompressedFrameGroup->frames[0] || nullptr == pFramegroup->frames[phaseIndex])
		{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "pFrame is nullptr, index: %d", phaseIndex);
			return  XN_STATUS_NULL_INPUT_PTR;
		}
	...
	if (XN_STATUS_OK != ret)
	{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "UnpackCompressedFrameGroup fail %d", ret);
		return  ret;
	}
	return  XN_STATUS_OK;
}

XnStatus XnGaeaSensor::UnpackCompressedFrameGroup(ObFrameGroup *pCompressedFrameGroup, ObFrameGroup *pFramegroup)
{
	if (nullptr == pCompressedFrameGroup || nullptr == pFramegroup)
	{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "pFrameGroup is nullptr");
	...
	for (XnInt i = 0; i < pCompressedFrameGroup->real_count; ++i)
	{
		...
		{
			xnLogError(XN_MASK_GAEA_TOF_SENSOR, "pFrame is nullptr, index: %d", i);
			return  XN_STATUS_NULL_INPUT_PTR;
		}
		...
		{
		case 8:
			ret = UnpackRaw8Data((XnInt8*)inputFrame->data_offset, (XnInt16*)outputFrame->data_offset, inputFrame->height, inputFrame->width);
			ret = ObcInt8ToInt16((XnInt16*)outputFrame->data_offset, (XnInt16*)outputFrame->data_offset, inputFrame->height, inputFrame->width);
			break;
		case 10:
			ret = UnpackRaw10Data((XnInt8*)inputFrame->data_offset, (XnInt16*)outputFrame->data_offset, inputFrame->height, inputFrame->width);
			ret = ObcInt10ToInt16((XnInt16*)outputFrame->data_offset, (XnInt16*)outputFrame->data_offset, inputFrame->height, inputFrame->width);
			break;
		case 12:
			ret = UnpackRaw12Data((XnInt8*)inputFrame->data_offset, (XnInt16*)outputFrame->data_offset, inputFrame->height, inputFrame->width);
...
XnStatus XnGaeaSensor::SetIntegrationTime(uint32_t integration_time)
{
	XnStatus ret = SetProperty(INTEGRATION_TIME, &integration_time, sizeof(integration_time));
	return ret;
}

XnStatus XnGaeaSensor::SetDutyCycle(uint32_t cycle)
{
	XnStatus ret = SetProperty(DUTY_CYCLE, &cycle, sizeof(cycle));
	return ret;
}

XnStatus XnGaeaSensor::SetTriggerSignal()
{
	XnStatus ret = SetProperty(SOFTWARE_TRIGGER, nullptr, 0);
	return ret;
}

XnStatus XnGaeaSensor::SetBinningFactor(XnUInt32 binningFactor)
{
	XnUInt8 bm;
	...
	case 4:
		...
		break;
	default:
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "SetBinningFactor invalid binningFactor: %d", binningFactor);
	...
	if (XN_STATUS_OK != ret)
	{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "set_property BINNING_MODE fail %d", ret); 
...
XnStatus XnGaeaSensor::UpdateMode()
{
	XnStatus nRet = XN_STATUS_OK;
	//updateFrameInfo();
	nRet = XnTofSensor::UpdateMode();

	nRet = CalcSensorProperty();
...
XnStatus XnGaeaSensor::updateFrameInfo()
{
...
XnStatus XnGaeaSensor::ParseExtendedData(ObTofFrameInfo* tof_frames, XnInt index)
{
	XnStatus ret = 0;
	...
	//char datafile[100];
	//XnUInt64 nEnd = 0;
	//xnOSGetTimeStamp(&nEnd);
	////sprintf(datafile, "%s_%ld.dat", "D:\\test\\", nEnd);
	//sprintf(datafile, "%s_%ld.dat", "D:\\test\\", 1);
	//xnOSSaveFile(datafile, expand_data, kEbdLength);
	uint8_t output_mode = expand_data[2974];
	uint8_t stream_mode = expand_data[2976];
	tof_frames->frame_type = stream_mode;
	tof_frames->out_mode = output_mode;
	//xnLogError(XN_MASK_GAEA_TOF_SENSOR, "out_mode = %d frame_type = %d", tof_frames->out_mode, tof_frames->frame_type);
	return ret;
}

XnStatus XnGaeaSensor::CalcIRFramePleco(ObFrameGroup *p_phase_framegroup, uint16_t *p_IR_frame, int width, int height){
...
XnStatus XnGaeaSensor::GetIRFrameSize(const ObFrameGroup *p_phase_framegroup, int *width, int *height)
{
	...
	default:
	{
			   xnLogError(XN_MASK_GAEA_TOF_SENSOR, "unsupport sensor: %x", p_phase_framegroup->frames[0]->sensor_type);
...
XnStatus XnGaeaSensor::GetFrameGroup(const XnChar *GrabBuffer, ObFrameGroup *p_framegroup)
{
	...
	*/
	...
	if (m_freqMode == TOF_SINGLE_FREQUENCY_MODE)
	{
		...
		for (uint32_t i = 0; i < p_framegroup->real_count; ++i)
		{
			...
			if (ret < 0)
			{
				xnLogError(XN_MASK_GAEA_TOF_SENSOR, "ParserExpandData ret=%d  i= %d", ret, i);
				return  ret;
			}
...
XnStatus XnGaeaSensor::GetPhaseFrame(const XnChar* dothinInputBuf, const XnUInt32 dothinInputBufSize, XnInt16 *frameData, XnUInt32 *frameDataSize)
{
	...
	{
		xnLogError(XN_MASK_GAEA_TOF_SENSOR, "GetPhaseFrame fail, null input or output ptr");
		return XN_STATUS_NULL_INPUT_PTR;
	}
	xnOSSaveFile("D:\\test\\inputData.dat", dothinInputBuf, dothinInputBufSize);
	//xnOSSaveFile("D:\\test\\inputData.raw", dothinInputBuf, dothinInputBufSize);
	...
	{
		xnOSMemCopy(frameDataAddr + outputImageSize / m_phaseOutputCount * i, m_pFrameGroup->frames[i]->data_offset, outputImageSize / m_phaseOutputCount);

	}
	return  XN_STATUS_OK;
}

XnStatus XnGaeaSensor::GetIRFrame(const XnUChar *dothinInputBuf, const XnUInt32 dothinInputBufSize, XnUInt16 *frameData, XnUInt32 *frameDataSize)
{
	if (nullptr == dothinInputBuf || nullptr == frameData
		|| nullptr == m_pFrameGroup || nullptr == m_pFrameGroup->frames
		|| nullptr == m_pCompressedFrameGroup || nullptr == m_pCompressedFrameGroup->frames)
	...
	else if (frameInfo->frame_index == 1)
	{
		...
		else
		{
			...
			mGroupIndex = -1;
			xnLogError(XN_MASK_GAEA_TOF_SENSOR, "IR PhaseFrame Lost frameIndex %d", frameInfo->frame_index);
		}
	...
	else if (frameInfo->frame_index == 2)
	{
		...
		else
		{
			mGroupIndex = -1;
			mPhaseStep = MAX_PHASE_STEP;
			xnLogError(XN_MASK_GAEA_TOF_SENSOR, "PhaseFrame Lost frameIndex %d", frameInfo->frame_index);
		}
...
XnStatus XnGaeaSensor::GetDepthFrame(const XnUChar *dothinInputBuf, const XnUInt32 dothinInputBufSize, XnUInt16 *frameData, XnUInt32 *frameDataSize)
{
	if (nullptr == dothinInputBuf || nullptr == frameData
		|| nullptr == m_pFrameGroup || nullptr == m_pFrameGroup->frames
		|| nullptr == m_pCompressedFrameGroup || nullptr == m_pCompressedFrameGroup->frames)
	...
	else if (frameInfo->frame_index == 1)
	{
		...
		else
		{
			mPhaseStep = MAX_PHASE_STEP;
			mGroupIndex = -1;
			xnLogError(XN_MASK_GAEA_TOF_SENSOR, "PhaseFrame Lost group_index %d frameIndex %d", frameInfo->group_index, frameInfo->frame_index);
		}
...
XnStatus XnGaeaSensor::GetAIFrame(const XnUChar* dothinInputBuf, const XnUInt32 dothinInputBufSize, OniAIFrame* pAiFrame)
{
...
XnStatus XnGaeaSensor::setFrameResolution(int width, int height)
{
	m_phaseWidth = m_grabWidth = width;
	m_phaseHeight = height;
	m_grabHeight = (height + kGrabExtendedDataLine) * m_phaseInputCount;
	xnLogInfo(XN_MASK_GAEA_TOF_SENSOR, "setFrameResolution  width %d， height %d", width, height);
	return  XN_STATUS_OK;
}
