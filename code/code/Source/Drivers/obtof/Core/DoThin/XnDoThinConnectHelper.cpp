...
XnDothinConnectHelper::XnDothinConnectHelper() :dothin_id(-1), i2c_write(1)
{
	...
	xnOSMemSet(&frameInfo, 0, sizeof(FrameInfo));
}
...
XnStatus XnDothinConnectHelper::DeviceOpen(const char *pszDeviceName)
{
	...
	if (res != DT_ERROR_OK)
	{
		xnLogError(XN_MASK_DOTHIN_CONNECT, "Failed to open device : %d", res);
		return XN_STATUS_ERROR;
	}

	xnLogInfo(XN_MASK_DOTHIN_CONNECT, "Successful opening of device , dothin_devId: %d", dothin_id);

	return XN_STATUS_OK;
}

XnStatus XnDothinConnectHelper::DothinDeviceReset()
{
	XnInt res = DeviceReset(dothin_id);
	if (res != DT_ERROR_OK)
	{
		xnLogError(XN_MASK_DOTHIN_CONNECT, "Failed to reset device : %d", res);
		return XN_STATUS_ERROR;
	}

	xnLogInfo(XN_MASK_DOTHIN_CONNECT, "Successful set device , dothin_devId: %d", dothin_id);
...
XnStatus XnDothinConnectHelper::InitSensorConfig()
{
	...
	sensor_Config.height = 800;
	sensor_Config.type = 1;//7:D_MIPI_RAW12;6:D_MIPI_RAW10;0:D_RAW10;1:D_RAW8;
	sensor_Config.port = 1;//0:MIPI;1:DVP


	sensor_Config.pin = 2;
	sensor_Config.SlaveID = 0x10;
	...
	sensor_Config.FlagMask = 0x00;
	sensor_Config.FlagData = 0x00;

	sensor_Config.FlagReg1 = 0x0000;
	sensor_Config.FlagMask1 = 0x00;
	//sensor_Config.FlagMask1 = 0xff;

	sensor_Config.FlagData1 = 0;
	sensor_Config.outformat = 3;
	sensor_Config.mclk = 24;

	i2c_Config.avdd = 3300;
	i2c_Config.dovdd = 1800;
	i2c_Config.dvdd = 1200;
	i2c_Config.afvcc = 3300;
...
XnStatus XnDothinConnectHelper::InitDiskInfo()
{
	...
	/*first set sensor working condition....*/
	{
		...
		//first set pin definition...
		{
			...
			else if (sensor_Config.port == PORT_PARALLEL)
			{
				//D_RAW10
				/*pinDef[0] = 20;
				pinDef[1] = 20;
...
XnStatus XnDothinConnectHelper::SetConfigType(XnInt bits)
{
	...
	case 8: sensor_Config.type = D_RAW8;
        break;
    case 10: sensor_Config.type = D_MIPI_RAW10;
	//case 10: sensor_Config.type = D_RAW10;
        break;
...
XnStatus XnDothinConnectHelper::Restart(XnUInt32 XRes, XnUInt32 YRes, XnUInt32* disBit)
{
    ...
    */
    ...
    SetConfigType(*disBit);//根据XnTofSensor中m_dothinMipiPackBit设置接口类型：MIPI_RAW12 or MIPI_RAW10
...
XnStatus XnDothinConnectHelper::ReadFrame(XnChar* buf, XnInt32 GrabSize, unsigned long * framesize)
{
	if (dothin_id <0)
	{
...
XnStatus XnDothinConnectHelper::DeviceClose()
{
	if (dothin_id != -1)
	{
		XnInt rtn = CloseDevice(dothin_id);
		if (rtn != DT_ERROR_OK)
		{
