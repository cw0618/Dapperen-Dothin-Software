...
OniStatus XnOniDepthStream::convertC2DCoordinates(int colorX, int colorY, OniDepthPixel depthZ, int* pDepthX, int* pDepthY)
{

	// take video mode from the color stream
	OniVideoMode videoMode;
    XnStatus retVal = GetVideoMode(&videoMode);
	if (retVal != XN_STATUS_OK)
	{
		XN_ASSERT(FALSE);
		return ONI_STATUS_ERROR;
	...
	// translate it to the internal property
	XnPixelRegistration pixelArgs;
	pixelArgs.nImageX = colorX;
	pixelArgs.nImageY = colorY;
	pixelArgs.nDepthValue = depthZ;
	pixelArgs.nImageXRes = videoMode.resolutionX;
	pixelArgs.nImageYRes = videoMode.resolutionY;
	pixelArgs.nDepthX = 0;
	...
	retVal = getProperty(XN_STREAM_PROPERTY_PIXEL_C2D_REGISTRATION, &pixelArgs, &pixelArgsSize);
	if (ONI_STATUS_OK != retVal)
	{
		return ONI_STATUS_ERROR;
	}
	// take output
	*pDepthX = pixelArgs.nDepthX;
	*pDepthY = pixelArgs.nDepthY;

	return ONI_STATUS_OK;
}

OniStatus XnOniDepthStream::convertD2CCoordinates(int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY)
{
	...
	retVal = getProperty(XN_STREAM_PROPERTY_PIXEL_D2C_REGISTRATION, &pixelArgs, &pixelArgsSize);
	if (ONI_STATUS_OK != retVal)
	{
		return ONI_STATUS_ERROR;
	}
