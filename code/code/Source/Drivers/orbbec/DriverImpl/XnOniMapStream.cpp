...
{
    XnStatus nRetVal = XN_STATUS_OK;
	xnLogError(XN_MASK_DEVICE_SENSOR, "setVideoMode width= %d ,height= %d", pVideoMode->resolutionX, pVideoMode->resolutionY);
	...
	xnLogError(XN_MASK_DEVICE_SENSOR, "before set xres");
    XnPropertySetAddIntProperty(&props, m_strType, XN_STREAM_PROPERTY_X_RES, pVideoMode->resolutionX);
	xnLogError(XN_MASK_DEVICE_SENSOR, "before set yres");
	...
	xnLogError(XN_MASK_DEVICE_SENSOR, "before BatchConfig");
    nRetVal = m_pSensor->BatchConfig(&props);
	xnLogError(XN_MASK_DEVICE_SENSOR, "after BatchConfig");
    XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnOniMapStream::FillSupportedVideoModes()
{
    ...
    XN_IS_STATUS_OK(nRetVal);
	xnLogError(XN_MASK_DEVICE_SENSOR, "support mode count=%d",nCount);
