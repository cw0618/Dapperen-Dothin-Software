...
static XnIOImageFormats g_anAllowedRGBFormats[] =
{ 
    ...
    XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG,
    XN_IO_IMAGE_FORMAT_H264
};

static XnIOImageFormats g_anAllowedYUVFormats[] = 
{
    XN_IO_IMAGE_FORMAT_UNCOMPRESSED_YUV422,
    XN_IO_IMAGE_FORMAT_YUV422,
    XN_IO_IMAGE_FORMAT_H264,
...
static XnIOImageFormats g_anAllowedYUYVFormats[] =
{ 
    XN_IO_IMAGE_FORMAT_UNCOMPRESSED_YUYV,
    XN_IO_IMAGE_FORMAT_H264,
    XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG
...
static XnIOImageFormats g_anAllowedJPEGFormats[] =
{ 
    XN_IO_IMAGE_FORMAT_JPEG
};

static XnIOImageFormats g_anAllowedGray8Formats[] =
{
    XN_IO_IMAGE_FORMAT_BAYER,
    XN_IO_IMAGE_FORMAT_UNCOMPRESSED_BAYER
};

static XnIOImageFormats g_anAllowedH264Formats[] =
{
    XN_IO_IMAGE_FORMAT_H264
};

static XnIOImageFormats g_anAllowedMjpegFormats[] =
{
    XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG
};
...
void XnOniColorStream::GetAllowedOniOutputFormatForInputFormat(XnIOImageFormats inputFormat, OniPixelFormat *aOniFormats, int *nOniFormats)
{
    *nOniFormats = 0;
    ...
    for (XnUInt32 i = 0; i < sizeof(g_anAllowedH264Formats) / sizeof(XnIOImageFormats); ++i)
    {
        if (g_anAllowedH264Formats[i] == inputFormat)
        {
            aOniFormats[*nOniFormats] = ONI_PIXEL_FORMAT_H264;
            ++(*nOniFormats);
            break;
        }
    }

    for (XnUInt32 i = 0; i < sizeof(g_anAllowedMjpegFormats) / sizeof(XnIOImageFormats); ++i)
    {
        if (g_anAllowedMjpegFormats[i] == inputFormat)
        {
            aOniFormats[*nOniFormats] = ONI_PIXEL_FORMAT_MJPEG;
            ++(*nOniFormats);
            break;
        }
    }
...
XnBool XnOniColorStream::IsPreferredInputFormat(XnIOImageFormats inputFormat, XnIOImageFormats thanFormat, OniPixelFormat oniFormat)
{
    ...
    case ONI_PIXEL_FORMAT_H264:
        aAllowedFormats = g_anAllowedH264Formats;
        nAllowedFormats = sizeof(g_anAllowedH264Formats) / sizeof(g_anAllowedH264Formats[0]);
        break;
    default:
        return FALSE;
    }
