...
XnStatus XnDeviceSensorOpenInputThreads(XnDevicePrivateData* pDevicePrivateData)
{
    ...
    pDevicePrivateData->pSpecificAIUsb = (XnSpecificUsbDevice*)xnOSMallocAligned(sizeof(XnSpecificUsbDevice), XN_DEFAULT_MEM_ALIGN);
    pDevicePrivateData->pSpecificAIUsb->pDevicePrivateData = pDevicePrivateData;
    pDevicePrivateData->pSpecificAIUsb->pUsbConnection = &pDevicePrivateData->SensorHandle.AIConnection;
    pDevicePrivateData->pSpecificAIUsb->CurrState.State = XN_WAITING_FOR_CONFIGURATION;
    pDevicePrivateData->pSpecificAIUsb->nIgnoreBytes = (pDevicePrivateData->FWInfo.nFWVer >= XN_SENSOR_FW_VER_5_0) ? 0 : pDevicePrivateData->pSpecificAIUsb->nChunkReadBytes;
    ...
    if (usbInterface == XN_SENSOR_USB_INTERFACE_ISO_ENDPOINTS || usbInterface == XN_SENSOR_USB_INTERFACE_ISO_ENDPOINTS_LOW_DEPTH)
    {
        ...
        pDevicePrivateData->pSpecificAIUsb->nTimeout = XN_SENSOR_READ_THREAD_TIMEOUT_ISO;
    }
    else
    {
        ...
        pDevicePrivateData->pSpecificAIUsb->nTimeout = XN_SENSOR_READ_THREAD_TIMEOUT_BULK;
    }
    ...
    if (usbInterface == XN_SENSOR_USB_INTERFACE_BULK_ENDPOINTS)
    {
        ...
        pDevicePrivateData->pSpecificAIUsb->nChunkReadBytes = XN_SENSOR_USB_BODY_BUFFER_SIZE_MULTIPLIER_BULK * pDevicePrivateData->SensorHandle.AIConnection.nMaxPacketSize;
    }
    else if (pDevicePrivateData->pSensor->IsLowBandwidth())
    {
        ...
        pDevicePrivateData->pSpecificAIUsb->nChunkReadBytes = XN_SENSOR_USB_BODY_BUFFER_SIZE_MULTIPLIER_LOWBAND_ISO * pDevicePrivateData->SensorHandle.AIConnection.nMaxPacketSize;
    }
    else
    {
        ...
        pDevicePrivateData->pSpecificAIUsb->nChunkReadBytes = XN_SENSOR_USB_BODY_BUFFER_SIZE_MULTIPLIER_ISO * pDevicePrivateData->SensorHandle.AIConnection.nMaxPacketSize;
    }
    ...
    pDevicePrivateData->pSpecificAIUsb->nNumberOfBuffers = XN_SENSOR_USB_BODY_BUFFERS;
...
XnStatus XnDeviceSensorAllocateBuffers(XnDevicePrivateData* pDevicePrivateData)
{
    ...
    {
        pDevicePrivateData->SensorHandle.AIConnection.pUSBBuffer = (XnUInt8*)xnOSCallocAligned(XN_SENSOR_PROTOCOL_USB_BUFFER_SIZE, sizeof(XnUInt8), XN_DEFAULT_MEM_ALIGN);
        pDevicePrivateData->SensorHandle.AIConnection.nUSBBufferReadOffset = 0;
        pDevicePrivateData->SensorHandle.AIConnection.nUSBBufferWriteOffset = 0;
    }
    else
    {
        pDevicePrivateData->SensorHandle.AIConnection.pUSBBuffer = NULL;
        pDevicePrivateData->SensorHandle.AIConnection.nUSBBufferReadOffset = 0;
        pDevicePrivateData->SensorHandle.AIConnection.nUSBBufferWriteOffset = 0;
    }
...
XnStatus XnDeviceSensorFreeBuffers(XnDevicePrivateData* pDevicePrivateData)
{
    ...
    if (pDevicePrivateData->SensorHandle.AIConnection.pUSBBuffer != NULL)
    {
        XN_ALIGNED_FREE_AND_NULL(pDevicePrivateData->SensorHandle.AIConnection.pUSBBuffer);
    }
    ...
    if (pDevicePrivateData->pSpecificAIUsb != NULL)
    {
        XN_ALIGNED_FREE_AND_NULL(pDevicePrivateData->pSpecificAIUsb);
    }
