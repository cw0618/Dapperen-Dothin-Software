...
#define XN_MASK_SENSOR_IMAGE_STREAM                  "XnSensorImageStream"
...
XnSensorImageStream::XnSensorImageStream(const XnChar* StreamName, XnSensorObjects* pObjects)
    ...
    , m_Helper(pObjects)
    , m_InputFormat(XN_STREAM_PROPERTY_INPUT_FORMAT, "InputFormat", XN_IO_IMAGE_FORMAT_UNCOMPRESSED_YUV422)
...
XnStatus XnSensorImageStream::Init()
{
    ...
    if (!bModeFound)
    {
        xnLogWarning(XN_MASK_SENSOR_IMAGE_STREAM, "Default mode (res + FPS) is not supported by device. Changing defaults...");
...
XnStatus XnSensorImageStream::MapPropertiesToFirmware()
{
    ...
    XN_IS_STATUS_OK(nRetVal);;
    //m_AutoExposure.SetValue(GetFirmwareParams()->m_ImageAutoExposure.GetValue());
    ...
    XN_IS_STATUS_OK(nRetVal);;
    m_Exposure.SetValue(GetFirmwareParams()->m_ImageExposureBar.GetValue());

    nRetVal = m_Helper.MapFirmwareProperty(m_Gain, GetFirmwareParams()->m_ImageGain, TRUE);
    XN_IS_STATUS_OK(nRetVal);;
    m_Gain.SetValue(GetFirmwareParams()->m_ImageGain.GetValue());
...
XnStatus XnSensorImageStream::ValidateMode()
{
    ...
    case ONI_PIXEL_FORMAT_RGB888:
        if (nInputFormat != XN_IO_IMAGE_FORMAT_YUV422 &&
            ...
            nInputFormat != XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG &&
            nInputFormat != XN_IO_IMAGE_FORMAT_UNCOMPRESSED_BAYER &&
            nInputFormat != XN_IO_IMAGE_FORMAT_H264)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to RGB24!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_YUV422:
        if (nInputFormat != XN_IO_IMAGE_FORMAT_YUV422 &&
            ...
            nInputFormat != XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to YUV422!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_YUYV:
        if (nInputFormat != XN_IO_IMAGE_FORMAT_UNCOMPRESSED_YUYV &&
            ...
            nInputFormat != XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to YUYV!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_JPEG:
        if (nInputFormat != XN_IO_IMAGE_FORMAT_JPEG)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to JPEG!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_GRAY8:
        if (nInputFormat != XN_IO_IMAGE_FORMAT_UNCOMPRESSED_BAYER &&
            nInputFormat != XN_IO_IMAGE_FORMAT_BAYER)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to Gray8!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_H264:
        if (XN_IO_IMAGE_FORMAT_H264 != nInputFormat)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to H264!", nInputFormat);
        }
        break;
    case ONI_PIXEL_FORMAT_MJPEG:
        if (XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG != nInputFormat)
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Input format %d cannot be converted to mjpeg!", nInputFormat);
        }
        break;
    default:
        // we shouldn't have reached here. Theres a check at SetOutputFormat.
        XN_ASSERT(FALSE);
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Unsupported image output format: %d!", nOutputFormat);
    }
...
XnStatus XnSensorImageStream::SetActualRead(XnBool bRead)
{
    ...
    if ((XnBool)m_ActualRead.GetValue() != bRead)
    {
        if (bRead)
        {
            xnLogVerbose(XN_MASK_SENSOR_IMAGE_STREAM, "Creating USB image read thread...");
        ...
        {
            xnLogVerbose(XN_MASK_SENSOR_IMAGE_STREAM, "Shutting down USB image read thread...");
            xnUSBShutdownReadThread(GetHelper()->GetPrivateData()->pSpecificImageUsb->pUsbConnection->UsbEp);
        }
...
XnStatus XnSensorImageStream::SetInputFormat(XnIOImageFormats nInputFormat)
{
    ...
    case XN_IO_IMAGE_FORMAT_H264:
        break;
    default:
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Unknown image input format: %d", nInputFormat);
    }
...
XnStatus XnSensorImageStream::SetImageQuality(XnUInt32 /*nQuality*/)
{
    ...
    if (m_InputFormat.GetValue() != XN_IO_IMAGE_FORMAT_JPEG)
    {
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_UNSUPPORTED_PARAMETER, XN_MASK_SENSOR_IMAGE_STREAM, "Image quality is only supported when input format is JPEG");
    }
...
XnStatus XnSensorImageStream::SetCroppingMode(XnCroppingMode mode)
{
    ...
    case XN_CROPPING_MODE_SOFTWARE_ONLY:
        break;
    default:
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "Bad cropping mode: %u", mode);
    }
...
XnUInt32 XnSensorImageStream::CalculateExpectedSize()
{
    ...
    case XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG:
        ...
        break;
    default:
        XN_LOG_WARNING_RETURN(XN_STATUS_ERROR, XN_MASK_SENSOR_IMAGE_STREAM, "Does not know to calculate expected size for input format %d", m_InputFormat.GetValue());
    }

    return nExpectedImageBufferSize;
}

XnStatus XnSensorImageStream::CalcRequiredSize(XnUInt32* pnRequiredSize) const
{
...
XnStatus XnSensorImageStream::CreateDataProcessor(XnDataProcessor** ppProcessor)
{
    ...
    case XN_IO_IMAGE_FORMAT_JPEG:
        ...
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "invalid output format %d!", GetOutputFormat());
        }
        break;
    case XN_IO_IMAGE_FORMAT_UNCOMPRESSED_YUV422:
    ...
    case XN_IO_IMAGE_FORMAT_UNCOMPRESSED_BAYER:
        ...
        break;
    case XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG:
        if (ONI_PIXEL_FORMAT_YUYV == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_YUV422 == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_MJPEG == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_RGB888 == GetOutputFormat())
        {
            XN_VALIDATE_NEW_AND_INIT(pNew, XnImageFFmpegProcessor, this, &m_Helper, pBufferManager);
        }
        else
        {
            XN_LOG_WARNING_RETURN(XN_STATUS_BAD_PARAM, XN_MASK_SENSOR_IMAGE_STREAM, "invalid output format %d!", GetOutputFormat());
        ...
        break;
    case XN_IO_IMAGE_FORMAT_H264:
        if (ONI_PIXEL_FORMAT_H264 == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_YUYV == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_YUV422 == GetOutputFormat() ||
            ONI_PIXEL_FORMAT_RGB888 == GetOutputFormat())
        {
            XN_VALIDATE_NEW_AND_INIT(pNew, XnImageFFmpegProcessor, this, &m_Helper, pBufferManager);
