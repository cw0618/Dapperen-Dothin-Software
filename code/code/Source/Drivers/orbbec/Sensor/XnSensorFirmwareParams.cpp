...
XnSensorFirmwareParams::XnSensorFirmwareParams(XnFirmwareInfo* pInfo, XnFirmwareCommands* pCommands)
    ...
    , m_freqMode(ONI_SINGLE_FREQ_NOSHUFFLE, "FrequencyMode")
    ...
    , m_AIFPS(0, "AIFPS")
    , m_AIFormat(0, "AIFormat")
    , m_AIMirror(0, "AIMirror")
    , m_AIResolution(0, "AIResolution")
    , m_phaseFPS(0, "PhaseFPS")
    , m_phaseFormat(0, "PhaseFormat")
    , m_phaseMirror(0, "PhaseMirror")
    , m_phaseResolution(0, "PhaseResolution")
    , m_phaseCropMode(0, "PhaseCropEnabled")
    , m_phaseCropSizeX(0, "PhaseCropSizeX")
    , m_phaseCropSizeY(0, "PhaseCropSizeY")
    , m_phaseCropOffsetX(0, "PhaseCropOffsetX")
    , m_phaseCropOffsetY(0, "PhaseCropOffsetY")
    , m_AllFirmwareParams()
    , m_pInfo(pInfo)
...
XnStatus XnSensorFirmwareParams::Init()
{
    ...
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_Stream3Mode,              PARAM_GENERAL_STREAM3_MODE);
    XN_IS_STATUS_OK(nRetVal);
    ...
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_freqMode,                 PARAM_FREQUENCY_MODE);
    XN_IS_STATUS_OK(nRetVal);
    ...
    /// AI
    nRetVal = AddFirmwareParam(     m_AIFPS,                     PARAM_AI_FPS);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_AIFormat,                  PARAM_AI_FORMAT,                        XN_SENSOR_FW_VER_5_2,     XN_SENSOR_FW_VER_UNKNOWN,       XN_IO_AI_FORMAT_JOINT_2D);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_AIMirror,                  PARAM_AI_MIRROR,                        XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       FALSE);
    XN_IS_STATUS_OK(nRetVal);
     nRetVal = AddFirmwareParam(    m_AIResolution,              PARAM_AI_RESOLUTION);
    XN_IS_STATUS_OK(nRetVal);

    /// Phase
    nRetVal = AddFirmwareParam(     m_phaseFPS,                  PARAM_PHASE_FPS);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseFormat,               PARAM_PHASE_FORMAT);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseMirror,               PARAM_PHASE_MIRROR,                        XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       FALSE);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseResolution,           PARAM_PHASE_RESOLUTION);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseCropMode,             PARAM_PHASE_CROP_MODE,                     XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       FALSE);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseCropSizeX,            PARAM_PHASE_CROP_SIZE_X,                   XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       0);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseCropSizeY,            PARAM_PHASE_CROP_SIZE_Y,                   XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       0);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseCropOffsetX,          PARAM_PHASE_CROP_OFFSET_X,                 XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       0);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = AddFirmwareParam(     m_phaseCropOffsetY,          PARAM_PHASE_CROP_OFFSET_Y,                 XN_SENSOR_FW_VER_5_0,     XN_SENSOR_FW_VER_UNKNOWN,       0);
    XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnSensorFirmwareParams::AddFirmwareParam(XnActualIntProperty& Property, XnUInt16 nFirmwareParam, XnFWVer nMinVer /* = XN_SENSOR_FW_VER_UNKNOWN */, XnFWVer nMaxVer /* = XN_SENSOR_FW_VER_UNKNOWN */, XnUInt16 nValueIfNotSupported /* = 0 */)
{
    ...
    XnChar csNewName[XN_DEVICE_MAX_STRING_LENGTH] = { 0 };
...
XnStatus XnSensorFirmwareParams::UpdateAllProperties()
{
    ...
    xnLogInfo(XN_MASK_DEVICE_SENSOR, "Reading all params from firmware...");
    for (XnFirmwareParamsHash::Iterator it = m_AllFirmwareParams.Begin(); it != m_AllFirmwareParams.End(); ++it)
    {
        ...
        {
            xnLogWarning(XN_MASK_DEVICE_SENSOR, "Update property from firmware failed: name (%s), value (%d).",
                param.pProperty->GetName(), param.nFirmwareParam);
            //return nRetVal;
        }
    }
    xnLogInfo(XN_MASK_DEVICE_SENSOR, "Firmware params were updated.");
