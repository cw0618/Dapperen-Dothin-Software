...
XnStatus XnSensorFixedParams::Init()
{
	...
	{
        if (m_pDevicePrivateData->Version.nMajor >= 5 &&
            m_pDevicePrivateData->Version.nMinor >= 8 &&
            m_pDevicePrivateData->Version.nBuild >= 24)
        {
            OniSerialNumberMap serialMap = { ONI_SERIAL_DEVICE, 0 };
            nRetVal = XnHostProtocolGetGeneralSerialNumber(m_pDevicePrivateData, &serialMap);
            XN_IS_STATUS_OK(nRetVal);
            xnOSStrCopy(m_strSensorSerial, serialMap.serial, XN_DEVICE_MAX_STRING_LENGTH);
        }
        else
        {
