...
{
    m_pIrDataBuf = new OniIRPixel[XN_MAX_RES_WIDTH * XN_MAX_RES_HEIGHT];
}

XnIRProcessor::~XnIRProcessor()
{
    if (NULL != m_pIrDataBuf)
    {
        delete[] m_pIrDataBuf;
    }
}
...
//superD1
XnStatus XnIRProcessor::SuperD1Unpack10to16(const XnUInt8* pcInput, const XnUInt32 nInputSize, XnUInt16* pnOutput, XnUInt32* pnActualRead, XnUInt32* pnOutputSize)
{
    XnInt32 cInput = 0;
    const XnUInt8* pOrigInput = pcInput;
...
void XnIRProcessor::ProcessFramePacketChunk(const XnSensorProtocolResponseHeader* /*pHeader*/, const XnUChar* pData, XnUInt32 /*nDataOffset*/, XnUInt32 nDataSize)
{
    ...
    if (m_ContinuousBuffer.GetSize() != 0)
    {
        ...
        if (m_ContinuousBuffer.GetSize() == XN_INPUT_ELEMENT_SIZE)
        {
            ...
            status = Unpack10to16(m_ContinuousBuffer.GetData(), XN_INPUT_ELEMENT_SIZE, (XnUInt16*)pWriteBuffer->GetUnsafeWritePointer(), &nActualRead, &nOutputSize);
    ...
    status = Unpack10to16(pData, nDataSize, (XnUInt16*)pWriteBuffer->GetUnsafeWritePointer(), &nActualRead, &nOutputSize);
    if (XN_STATUS_OK != status)
    {
        WriteBufferOverflowed();
    }
...
void SuperD1IRto888(XnUInt16* pInput, XnUInt32 nInputSize, XnUInt8* pOutput, XnUInt32* pnOutputSize)
{
    XnUInt16* pInputEnd = pInput + nInputSize;
    XnUInt8* pOutputOrig = pOutput;
    XnUInt8* pOutputEnd = pOutput + *pnOutputSize;
