...
#define XN_SHIFTS_12_BIT_MAX_SHIFT							4095
#define XN_SHIFTS_10_BIT_MAX_SHIFT							1023
#define XN_SHIFTS_PIXEL_SIZE_FACTOR							1
#define XN_SHIFTS_PARAM_COEFFICIENT							4
#define XN_SHIFTS_12_BIT_PARAM_COEFFICIENT					8
#define XN_SHIFTS_10_BIT_PARAM_COEFFICIENT					4
#define XN_SHIFTS_SHIFT_SCALE								10

#define XN_DEPTH_STREAM_AGC_NUMBER_OF_BINS					4

#define XN_MASK_SENSOR_DEPTH_STREAM                         "XnSensorDepthStream"
	...
	m_hReferenceSizeChangedCallback(NULL)
{
	...
	m_InputFormat.SetAlwaysSet(TRUE);
}

void XnSensorDepthStream::SetDriverConfig(char* path, int size){
...
XnStatus XnSensorDepthStream::Init()
{
    ...
    // m_DepthRegistration.UpdateGetCallback(GetRegistrationCallback, this);
	...
	int nMx6000Id = m_Helper.GetPrivateData()->ChipInfo.nChipVer;
	if (XN_SENSOR_CHIP_VER_DUAL_MX6000 == nMx6000Id)
	{
		//	Fov_h = atan(image_width*0.5 / fx;
		//	Fov_v = atan(image_height*0.5 / fy);
		ObContent_t params;
		xnOSMemSet(&params.HOST.virCam, 0, sizeof(params.HOST.virCam));
		XnInt32 nTimesLeft = 3;
		nRetVal = XN_STATUS_ERROR;
		//retry
		while (nTimesLeft > 0)
		{
			nRetVal = m_Helper.GetPrivateData()->pSensor->GetDualCameraParam(params);
			nTimesLeft--;
		...
		if (nRetVal == XN_STATUS_OK)
		{
			nRetVal = m_HorizontalFOV.UnsafeUpdateValue(2*atan(params.HOST.virCam.w*0.5 / params.HOST.virCam.fx));
			XN_IS_STATUS_OK(nRetVal);
			nRetVal = m_VerticalFOV.UnsafeUpdateValue(2*atan(params.HOST.virCam.h*0.5 / params.HOST.virCam.fy));
			XN_IS_STATUS_OK(nRetVal);
		}
	...
	else
	{
		...
		xnOSMemSet(&params, 0, sizeof(params));

		m_Helper.GetPrivateData()->pSensor->GetCameraParam(params);

		XnDouble fZPPS = m_Helper.GetFixedParams()->GetZeroPlanePixelSize();
		XnDouble nZPD = m_Helper.GetFixedParams()->GetZeroPlaneDistance();

		nRetVal = m_HorizontalFOV.UnsafeUpdateValue(2 * atan(fZPPS*XN_SXGA_X_RES / 2 / nZPD));
		XN_IS_STATUS_OK(nRetVal);

		switch (m_Helper.GetFixedParams()->GetDepthCmosType()){
		case XN_DEPTH_CMOS_OV9282:
		case XN_DEPTH_CMOS_AR0144:{
			nRetVal = m_VerticalFOV.UnsafeUpdateValue(2 * atan(fZPPS*XN_VGAS_Y_RES * 2 / 2 / nZPD));
			XN_IS_STATUS_OK(nRetVal);
		}
								  break;
		default:{
    ...
    // Metadata
    XnInt32 metadata = 0;
    nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "Metadata", &metadata);
    if (nRetVal == XN_STATUS_OK)
    {
	...
	// read min depth value from config file
	int nMinDepthValue = -1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "MinDepthValue", &nMinDepthValue); //MinDepthValue
	if (nRetVal == XN_STATUS_OK){
		nRetVal = MinDepthProperty().UnsafeUpdateValue(nMinDepthValue);
		XN_IS_STATUS_OK(nRetVal);
	}

	// read max depth value from config file
	int nMaxDepthValue =-1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "MaxDepthValue", &nMaxDepthValue); //MaxDepthValue
	if (nRetVal == XN_STATUS_OK){
		nRetVal = MaxDepthProperty().UnsafeUpdateValue(nMaxDepthValue);
		XN_IS_STATUS_OK(nRetVal);
	}

	// softfilter enable
	int nSoftFilter = -1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "SoftFilter", &nSoftFilter);
	if (nRetVal == XN_STATUS_OK){
		nRetVal = SoftFilterProperty().UnsafeUpdateValue(nSoftFilter);
		XN_IS_STATUS_OK(nRetVal);
	}

	int nDepthRotate = -1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "DepthRotate", &nDepthRotate);
	if (nRetVal == XN_STATUS_OK){
		nRetVal = DepthRotateProperty().UnsafeUpdateValue(nDepthRotate);
		XN_IS_STATUS_OK(nRetVal);
	}

	// MaxDiff
	int nMaxDiff = -1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "MaxDiff", &nMaxDiff);
	if (nRetVal == XN_STATUS_OK){
		nRetVal = DepthMaxDiffProperty().UnsafeUpdateValue(nMaxDiff);
		XN_IS_STATUS_OK(nRetVal);
	}

	// MaxSpeckleSize
	int nMaxSpeckleSize = -1;
	nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "MaxSpeckleSize", &nMaxSpeckleSize);
	if (nRetVal == XN_STATUS_OK){
		nRetVal = DepthMaxSpeckleSizeProperty().UnsafeUpdateValue(nMaxSpeckleSize);
		XN_IS_STATUS_OK(nRetVal);
	}

    int value = -1;
	bool isMX6000 = m_Helper.GetPrivateData()->ChipInfo.nChipVer == XN_SENSOR_CHIP_VER_MX6000;

    nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "ReadParams", &value);
    if(isMX6000 && nRetVal == XN_STATUS_OK && (value == 1)){

          nRetVal = xnOSReadIntFromINI(m_driverConfig, "Depth", "dPlaneDistance", &value); //ZPD
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = ZeroPlaneDistanceProperty().UnsafeUpdateValue(value);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  XnDouble dValue = -0.1f;
		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dPlanePixelSize", &dValue);//ZPPS
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = ZeroPlanePixelSizeProperty().UnsafeUpdateValue(dValue);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dEmitterDcmosDistance", &dValue); //LDDIS
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = EmitterDCmosDistanceProperty().UnsafeUpdateValue(dValue);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = GetDCmosRCmosDistanceProperty().UnsafeUpdateValue(m_Helper.GetFixedParams()->GetDCmosRCmosDistance()); //DCRCDIS
		  XN_IS_STATUS_OK(nRetVal);

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dhorizontalFov", &dValue);
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = m_HorizontalFOV.UnsafeUpdateValue(dValue);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dverticalFov", &dValue);
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = m_VerticalFOV.UnsafeUpdateValue(dValue);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "ldmpCurrent", &dValue);
		  if (nRetVal == XN_STATUS_OK){
			  nRetVal = XnHostProtocolSetLaserCurrent(m_Helper.GetPrivateData(), (XnUInt32)dValue);
			  XN_IS_STATUS_OK(nRetVal);
		  }

	}else if((1 == value) && m_Helper.GetPrivateData()->ChipInfo.nChipVer == XN_SENSOR_CHIP_VER_DUAL_MX6000){
		  double dTemp = 0.0f;
		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "focallength", &dTemp); //ZPD
		  if(nRetVal == XN_STATUS_OK){
		     DualFocallengthProperty().UnsafeUpdateValue(dTemp);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dualCoeffD", &dTemp);//dualcoeffdisparity
		  if (nRetVal == XN_STATUS_OK){
			  nRetVal = DualCoeffDisparityProperty().UnsafeUpdateValue(dTemp);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dPlanePixelSize", &dTemp);//ZPPS
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = ZeroPlanePixelSizeProperty().UnsafeUpdateValue(dTemp);
			  XN_IS_STATUS_OK(nRetVal);
		  }

		  nRetVal = xnOSReadDoubleFromINI(m_driverConfig, "Depth", "dEmitterDcmosDistance", &dTemp); //LDDIS
		  if(nRetVal == XN_STATUS_OK){
			  nRetVal = EmitterDCmosDistanceProperty().UnsafeUpdateValue(dTemp);
			  XN_IS_STATUS_OK(nRetVal);
		  }
    ...
    xnLogVerbose(XN_MASK_SENSOR_DEPTH_STREAM, "cmostype: %d m_VerticalFOV: %lf, m_HorizontalFOV: %lf",
			m_Helper.GetFixedParams()->GetDepthCmosType(), m_VerticalFOV.GetValue(), m_HorizontalFOV.GetValue());
    ...
    /// A patch to fix the wrong buffer size, although the resolution remains the same in case of mask mode,
    /// the size of mask data needs to be taken into consideration, so we need to reallocate the cache.
    /// RegisterRequiredSizeProperty(&m_InputFormat);
    /// XN_IS_STATUS_OK(nRetVal);
	...
	if (m_Helper.GetFirmwareVersion() > XN_SENSOR_FW_VER_5_3 && !ONI_PLATFORM_SUPPORT_MARIO)
	{
...
{
    XnStatus nRetVal = m_Helper.MapFirmwareProperty(m_InputFormat, GetFirmwareParams()->m_DepthFormat, FALSE);
	XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnSensorDepthStream::SetActualRead(XnBool bRead)
{
	...
	if ((XnBool)m_ActualRead.GetValue() != bRead)
	{
		if (bRead)
		{
            xnLogVerbose(XN_MASK_SENSOR_DEPTH_STREAM, "Creating USB depth read thread...");
		...
		{
            xnLogVerbose(XN_MASK_SENSOR_DEPTH_STREAM, "Shutting down USB depth read thread...");
			xnUSBShutdownReadThread(GetHelper()->GetPrivateData()->pSpecificDepthUsb->pUsbConnection->UsbEp);
...
XnStatus XnSensorDepthStream::SetOutputFormat(OniPixelFormat nOutputFormat)
{
	...
	case ONI_PIXEL_FORMAT_DEPTH_100_UM:
		...
		break;
	default:
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Unsupported depth output format: %d", nOutputFormat);
	}
	XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnSensorDepthStream::SetMirror(XnBool bIsMirrored)
{
	...
	XnUInt16 nDevicePid = m_Helper.GetPrivateData()->pSensor->GetDevicePID();
	if (nDevicePid == Tornado_D2_PID || nDevicePid == Tornado_D2_PRO_PID || BAIDU_Atlas == nDevicePid)
	{
		//TornadoD2 use sdk mirror
		bFirmwareMirror = FALSE;
	}
...
XnStatus XnSensorDepthStream::SetInputFormat(XnIODepthFormats nInputFormat)
{
	...
	if (nDevicePid == Tornado_D2_PID || nDevicePid == Tornado_D2_PRO_PID || BAIDU_Atlas == nDevicePid)
	{
		//TornadoD2 depth use 10bit
		nInputFormat = XN_IO_DEPTH_FORMAT_UNCOMPRESSED_10_BIT;
	}
	...
	case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_11_BIT:
	{
		if (m_Helper.GetFirmwareVersion() < XN_SENSOR_FW_VER_4_0)
		{
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_UNSUPPORTED_MODE, XN_MASK_SENSOR_DEPTH_STREAM, "11-bit depth is not supported on this sensor!");
		}

		nRetVal = MaxShiftProperty().UnsafeUpdateValue(XN_SHIFTS_MAX_SHIFT);
		XN_IS_STATUS_OK(nRetVal);
		nRetVal = ParamCoefficientProperty().UnsafeUpdateValue(XN_SHIFTS_PARAM_COEFFICIENT);
		XN_IS_STATUS_OK(nRetVal);

		if (m_Helper.GetPrivateData()->ChipInfo.nChipVer == XN_SENSOR_CHIP_VER_DUAL_MX6000)
		{
			XnFloat fDisparityCoeff = 0.0;
			nRetVal = XnHostProtocolGetDisparityCoeff(m_Helper.GetPrivateData(), (XnUInt16)nInputFormat, fDisparityCoeff);

			if (nRetVal !=XN_STATUS_OK)
			{
				fDisparityCoeff = 1313.5;
			}

			nRetVal = DualCoeffDisparityProperty().UnsafeUpdateValue(fDisparityCoeff);
			XN_IS_STATUS_OK(nRetVal);
		}
	...
	case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_12_BIT:
	{
		if (m_Helper.GetFirmwareVersion() < XN_SENSOR_FW_VER_4_0)
		{
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_UNSUPPORTED_MODE, XN_MASK_SENSOR_DEPTH_STREAM, "12-bit depth is not supported on this sensor!");
		}

		nRetVal = MaxShiftProperty().UnsafeUpdateValue(XN_SHIFTS_12_BIT_MAX_SHIFT);
		XN_IS_STATUS_OK(nRetVal);
		nRetVal = ParamCoefficientProperty().UnsafeUpdateValue(XN_SHIFTS_12_BIT_PARAM_COEFFICIENT);
		XN_IS_STATUS_OK(nRetVal);
	...
	break;
	case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_10_BIT:
	{
        xnLogInfo(XN_MASK_SENSOR_DEPTH_STREAM, "10-bit depth format");
		nRetVal = MaxShiftProperty().UnsafeUpdateValue(XN_SHIFTS_10_BIT_MAX_SHIFT);
		XN_IS_STATUS_OK(nRetVal);
		nRetVal = ParamCoefficientProperty().UnsafeUpdateValue(XN_SHIFTS_10_BIT_PARAM_COEFFICIENT);
		XN_IS_STATUS_OK(nRetVal);
		break;
	}
	default:
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Unknown depth input format: %d", nInputFormat);
	}
...
XnStatus XnSensorDepthStream::SetCroppingMode(XnCroppingMode mode)
{
	...
	case XN_CROPPING_MODE_SOFTWARE_ONLY:
		break;
	default:
        XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Bad cropping mode: %u", mode);
	}
...
XnStatus XnSensorDepthStream::GetRegistration(OniImageRegistrationMode* pMode)
{
    return XnHostProtocolGetParam(m_Helper.GetPrivateData(), PARAM_GENERAL_REGISTRATION_ENABLE, *(XnUInt16*)pMode);
}
...
{
    XN_RET_IF_NULL(pnRequiredSize, XN_STATUS_NULL_INPUT_PTR);

    *pnRequiredSize = GetXRes() * GetYRes();

    /// When cropping is turned on, actual image size is smaller.
    const OniCropping* pCropping = GetCropping();
    if (pCropping->enabled)
        *pnRequiredSize = pCropping->width * pCropping->height;

    *pnRequiredSize = *pnRequiredSize * GetBytesPerPixel() + GetXRes() * GetBytesPerPixel() * GetMetadataLine();
    return XN_STATUS_OK;
}
...
XnStatus XnSensorDepthStream::CreateDataProcessor(XnDataProcessor** ppProcessor)
{
	...
	case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_12_BIT:
		...
		break;
	case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_10_BIT:
		XN_VALIDATE_NEW_AND_INIT(pNew, XnPacked10DepthProcessor, this, &m_Helper, pBufferManager);
		break;
    case XN_IO_DEPTH_FORMAT_UNCOMPRESSED_DEPTH_16_BIT:
        XN_VALIDATE_NEW_AND_INIT(pNew, XnDepthUnpacked16Processor, this, &m_Helper, pBufferManager);
        break;
	default:
        xnLogError(XN_MASK_SENSOR_DEPTH_STREAM, "Not supported input format (%d)", m_InputFormat.GetValue());
...
XnStatus XnSensorDepthStream::DecideFirmwareRegistration(XnBool bRegistration, XnProcessingType registrationType, XnResolutions nRes)
{
	...
	if (bFirmwareRegistration)
	{
		...
		case XN_PROCESSING_HARDWARE:
			...
			{
                XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Sensor does not support hardware registration for current configuration!");
			}
			break;
		case XN_PROCESSING_SOFTWARE:
			if (GetFPS() == 60)
			{
                XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Software registration is not supported in 60 FPS mode!");
			}
		...
		case XN_PROCESSING_DONT_CARE:
			...
			break;
		default:
            XN_LOG_ERROR_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Unknown registration type: %d", registrationType);
		}
	}
...
XnStatus XnSensorDepthStream::DecidePixelSizeFactor()
{
	...
	case XN_RESOLUTION_VGA:
		...
		break;
	default:
        XN_LOG_WARNING_RETURN(XN_STATUS_ERROR, XN_MASK_SENSOR_DEPTH_STREAM, "Can't resolve pixel size for reference resolution %llu", GetFirmwareParams()->m_ReferenceResolution.GetValue());
	}
...
XnStatus XnSensorDepthStream::PopulateSensorCalibrationInfo()
{
	...
	m_calibrationInfo.params1080.rgbRegXRes = 1280;
	m_calibrationInfo.params1080.rgbRegYRes = 1024;
	...
	XN_IS_STATUS_OK(nRetVal);
	//astrapro SM usb timeout
	/*
	nRetVal = XnHostProtocolAlgorithmParams(m_Helper.GetPrivateData(), XN_HOST_PROTOCOL_ALGORITHM_REGISTRATION, &m_calibrationInfo.params1080.registrationInfo_SXGA, sizeof(m_calibrationInfo.params1080.registrationInfo_SXGA), XN_RESOLUTION_1280_800, 5);
	if (nRetVal != XN_STATUS_OK)
	{
		xnOSMemSet(&m_calibrationInfo.params1080.registrationInfo_SXGA, 0, sizeof(m_calibrationInfo.params1080.registrationInfo_SXGA));
	}
	*/
...
XnStatus XN_CALLBACK_TYPE XnSensorDepthStream::ReadAGCBinsFromFile(XnGeneralProperty* pSender, const XnChar* csINIFile, const XnChar* csSection)
{
	...
	for (XnUInt32 nBin = 0; nBin < XN_DEPTH_STREAM_AGC_NUMBER_OF_BINS; ++nBin)
	{
		...
		{
			// we have only one
            XN_LOG_WARNING_RETURN(XN_STATUS_DEVICE_BAD_PARAM, XN_MASK_SENSOR_DEPTH_STREAM, "Bin %d should have both min and max values!", nBin);
		}
	}
...
XnStatus XN_CALLBACK_TYPE XnSensorDepthStream::GetRegistrationCallback(const XnActualIntProperty* pSender, XnUInt64* pValue, void* pCookie)
{
    ...
    XnSensorDepthStream* pThis = (XnSensorDepthStream*)pCookie;
    return pThis->GetRegistration((OniImageRegistrationMode*)pValue);
}
