...
{
    xnLogInfo(XN_MASK_DEVICE_IO, "Connecting to USB device...");

    // try to open the device
    xnLogInfo(XN_MASK_DEVICE_IO, "Trying to open sensor '%s'...", strPath);
    XnStatus nRetVal = xnUSBOpenDeviceByPath(strPath, &m_pSensorHandle->USBDevice);
    XN_IS_STATUS_OK(nRetVal);

    // on older firmwares, control was sent over BULK endpoints. Check if this is the case
    xnLogInfo(XN_MASK_DEVICE_IO, "Trying to open endpoint 0x4 for control out (for old firmwares)...");
    ...
    else
    {
        ...
        xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoint 0x85 for control in...");
    ...
    xnLogInfo(XN_MASK_DEVICE_IO, "Trying to open endpoint 0x1 for control bulk out...");
    nRetVal = xnUSBOpenEndPoint(m_pSensorHandle->USBDevice, 0x1, XN_USB_EP_BULK, XN_USB_DIRECTION_OUT, &m_pSensorHandle->ControlConnectionBulk.ControlOutConnectionEp);
    if (XN_STATUS_OK == nRetVal)
        m_pSensorHandle->ControlConnectionBulk.bIsBulk = TRUE;
    else
        xnLogWarning(XN_MASK_DEVICE_IO, "Open endpoint 0x1 failed to (%s)...", xnGetStatusString(nRetVal));
...
XnStatus XnSensorIO::OpenDataEndPoints(XnSensorUsbInterface nInterface, const XnFirmwareInfo& fwInfo)
{
    ...
    if (nInterface != XN_SENSOR_USB_INTERFACE_DEFAULT)
    {
        ...
        xnLogInfo(XN_MASK_DEVICE_IO, "Setting USB alternative interface to %d...", nAlternativeInterface);
        nRetVal = xnUSBSetInterface(m_pSensorHandle->USBDevice, 0, nAlternativeInterface);
        XN_IS_STATUS_OK(nRetVal);
    }

    xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoints...");
    ...
    xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoint 0x81 for depth...");
    ...
    {
        bNewUSB = FALSE;
        xnLogWarning(XN_MASK_DEVICE_IO, "Endpoint 0x81 does not exist. Trying old USB: Opening 0x82 for depth...");
    ...
    else
    {
        ...
        if (bIsISO)
        {
            xnLogInfo(XN_MASK_DEVICE_IO, "Depth endpoint is isochronous.");
        }
        else
        {
            xnLogInfo(XN_MASK_DEVICE_IO, "Depth endpoint is bulk.");
        }
    }
    ...
    xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoint 0x%hx for image...", nImageEP);
    ...
    if (bIsISO)
    {
        xnLogInfo(XN_MASK_DEVICE_IO, "Image endpoint is isochronous.");
    }
    else
    {
        xnLogInfo(XN_MASK_DEVICE_IO, "Image endpoint is bulk.");
    }
    ...
    xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoint 0x%hx for misc...", nMiscEP);
    ...
    if (nRetVal == XN_STATUS_USB_ENDPOINT_NOT_FOUND)
    {
        ...
        m_bMiscSupported = FALSE;
        xnLogWarning(XN_MASK_DEVICE_IO, "Misc endpoint is not supported...");
    }
    else if (nRetVal == XN_STATUS_OK)
    {
        ...
        if (bIsISO)
        {
            xnLogInfo(XN_MASK_DEVICE_IO, "Misc endpoint is isochronous.");
        }
        else
        {
            xnLogInfo(XN_MASK_DEVICE_IO, "Misc endpoint is bulk.");
        }
    }
    else
    {
    ...
    /// AI
    XnUInt16 nAIEP = 0x83;
    xnLogInfo(XN_MASK_DEVICE_IO, "Opening endpoint 0x%hx for AI...", nAIEP);
    nRetVal = xnUSBOpenEndPoint(m_pSensorHandle->USBDevice, nAIEP, XN_USB_EP_BULK, XN_USB_DIRECTION_IN, &m_pSensorHandle->AIConnection.UsbEp);
    if (XN_STATUS_USB_WRONG_ENDPOINT_TYPE == nRetVal)
    {
        bIsISO = TRUE;
        nRetVal = xnUSBOpenEndPoint(m_pSensorHandle->USBDevice, nAIEP, XN_USB_EP_ISOCHRONOUS, XN_USB_DIRECTION_IN, &m_pSensorHandle->AIConnection.UsbEp);
    }
    if (XN_STATUS_OK == nRetVal)
    {
        m_bAISupported = TRUE;
        m_pSensorHandle->MiscConnection.bIsOpen = TRUE;
        nRetVal = xnUSBGetEndPointMaxPacketSize(m_pSensorHandle->AIConnection.UsbEp, &m_pSensorHandle->AIConnection.nMaxPacketSize);
        XN_IS_STATUS_OK(nRetVal);
        xnLogInfo(XN_MASK_DEVICE_IO, "Body endpoint type: isochronous(%d) or bulk(%d).", bIsISO, !bIsISO);
    }
    else
    {
        /// Firmware does not support AI...
        m_bAISupported = FALSE;
        m_pSensorHandle->AIConnection.bIsOpen = FALSE;
        xnLogWarning(XN_MASK_DEVICE_IO, "AI endpoint 0x%hx is not supported...", nAIEP);
    }

    xnLogInfo(XN_MASK_DEVICE_IO, "All endpoints opened...");
...
XnStatus XnSensorIO::CloseDevice()
{
    ...
    if (m_pSensorHandle->MiscConnection.bIsOpen)
    {
        xnLogInfo(XN_MASK_DEVICE_SENSOR, "Shutting down USB misc read thread...");
    ...
    if (m_pSensorHandle->AIConnection.bIsOpen)
    {
        xnLogInfo(XN_MASK_DEVICE_SENSOR, "Shutting down USB AI read thread...");
        xnUSBShutdownReadThread(m_pSensorHandle->AIConnection.UsbEp);
        if (NULL != m_pSensorHandle->AIConnection.UsbEp)
        {
            nRetVal = xnUSBCloseEndPoint(m_pSensorHandle->AIConnection.UsbEp);
            XN_IS_STATUS_OK(nRetVal);
            m_pSensorHandle->AIConnection.UsbEp = NULL;
        }
    }
    ...
    if (m_pSensorHandle->ControlConnectionBulk.bIsBulk)
    {
        if (NULL != m_pSensorHandle->ControlConnectionBulk.ControlOutConnectionEp)
        {
            nRetVal = xnUSBCloseEndPoint(m_pSensorHandle->ControlConnectionBulk.ControlOutConnectionEp);
            XN_IS_STATUS_OK(nRetVal);
            m_pSensorHandle->ControlConnectionBulk.ControlOutConnectionEp = NULL;
        }
    }
    ...
    xnLogInfo(XN_MASK_DEVICE_SENSOR, "Device closed successfully");
