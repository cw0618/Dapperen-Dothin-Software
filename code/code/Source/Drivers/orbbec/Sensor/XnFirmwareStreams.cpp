...
XnStatus XnFirmwareStreams::Init()
{
    ...
    // IR (currently uses the same processor due to IR & Color share one endpoint).
    tempData.pProcessorHolder = &m_ImageProcessor;
    ...
    // Phase (currently uses the same processor due to Depth & Phase share one endpoint).
    // tempData.strType = XN_STREAM_TYPE_PHASE;
    // tempData.pProcessorHolder = &m_DepthProcessor;
    // nRetVal = m_FirmwareStreams.Set(XN_STREAM_TYPE_PHASE, tempData);
    // XN_IS_STATUS_OK(nRetVal);

    // AI
    nRetVal = m_AIProcessor.Init();
    XN_IS_STATUS_OK(nRetVal);
    tempData.strType = XN_STREAM_TYPE_AI;
    tempData.pProcessorHolder = &m_AIProcessor;
    nRetVal = m_FirmwareStreams.Set(XN_STREAM_TYPE_AI, tempData);
    XN_IS_STATUS_OK(nRetVal);
    ...
    if (strcmp(strType, XN_STREAM_TYPE_DEPTH) == 0)
    {
        ...
        if (pIRStreamData->pOwner != NULL)
        {
                ...
                !(pIRStreamData->nRes == XN_RESOLUTION_SXGA && nRes == XN_RESOLUTION_VGA))
            {
                ...
                xnLogWarning(XN_MASK_DEVICE_SENSOR, "set depth stream to resolution %d when IR is set to resolution %d!", nRes, pIRStreamData->nRes);
            }
...
void XnFirmwareStreams::ProcessPacketChunk(XnSensorProtocolResponseHeader* pHeader, XnUChar* pData, XnUInt32 nDataOffset, XnUInt32 nDataSize)
{
    ...
    {
    // case XN_SENSOR_PROTOCOL_RESPONSE_PHASE_START:
    // case XN_SENSOR_PROTOCOL_RESPONSE_PHASE_BUFFER:
    // case XN_SENSOR_PROTOCOL_RESPONSE_PHASE_END:
    ...
    case XN_SENSOR_PROTOCOL_RESPONSE_DEPTH_END:
        pStreamProcessor = &m_DepthProcessor; ///< Depth & Phase share one endpoint.
        break;
    ...
    case XN_SENSOR_PROTOCOL_RESPONSE_IMAGE_END:
        pStreamProcessor = &m_ImageProcessor; ///< IR & Color share one endpoint.
        break;
    case XN_SENSOR_PROTOCOL_RESPONSE_AI_START:
    case XN_SENSOR_PROTOCOL_RESPONSE_AI_BUFFER:
    case XN_SENSOR_PROTOCOL_RESPONSE_AI_END:
        pStreamProcessor = &m_AIProcessor;
        break;
