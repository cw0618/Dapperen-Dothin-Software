...
#define XN_MASK_DEVICE_MODULE    "DeviceModule"
...
XnStatus XnDeviceModule::AddProperty(XnProperty* pProperty)
{
    ...
    {
        xnLogError(XN_MASK_DEVICE_MODULE, "Property (0x%x) already exists...\n", pProperty->GetId());
...
XnStatus XnDeviceModule::GetProperty(XnUInt32 propertyId, void* data, int* pDataSize) const
{
    ...
    case XN_PROPERTY_TYPE_STRING:
    {
        ...
        nRetVal = xnOSStrNCopy((XnChar*)data, strValue, xnOSStrLen(strValue), *pDataSize);
        if (nRetVal != XN_STATUS_OK)
        {
            // wrong size?
            XN_ASSERT(FALSE);
