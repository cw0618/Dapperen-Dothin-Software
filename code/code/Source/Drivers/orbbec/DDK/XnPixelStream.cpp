...
XnPixelStream::XnPixelStream(const XnChar* csType, const XnChar* csName, XnBool bAllowCustomResolutions)
    ...
    , m_metadataLine(XN_STREAM_PROPERTY_METADATA_LINE, "Metadata", 0)
...
XnStatus XnPixelStream::Init()
{
    ...
    m_metadataLine.UpdateSetCallback(SetMetadataLineCallback, this);
    ...
    XN_VALIDATE_ADD_PROPERTIES(this, &m_IsPixelStream, &m_Resolution, &m_XRes, &m_YRes,
        &m_BytesPerPixel, &m_Cropping, &m_SupportedModesCount, &m_SupportedModes, &m_metadataLine);
    ...
    nRetVal = RegisterRequiredSizeProperty(&m_metadataLine);
    XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnPixelStream::SetMetadataLine(const XnUInt32 line)
{
    return m_metadataLine.UnsafeUpdateValue(line);
}
...
{
    *pnRequiredSize = GetXRes() * (GetYRes() + GetMetadataLine()) * GetBytesPerPixel();
    return XN_STATUS_OK;
}
...
{
    return XnFormatsMirrorPixelData(
        GetOutputFormat(),
        (XnUChar*)pFrame->data + pFrame->stride * pFrame->extraLine,
        pFrame->dataSize - pFrame->stride * pFrame->extraLine,
        pFrame->width
        );
}
...
XnStatus XN_CALLBACK_TYPE XnPixelStream::SetMetadataLineCallback(XnActualIntProperty* /*pSender*/, XnUInt64 nValue, void* pCookie)
{
    XnPixelStream* pThis = (XnPixelStream*)pCookie;
    return pThis->SetMetadataLine((XnUInt32)nValue);
}
