...
XnStatus XnShiftToDepthUpdate(XnShiftToDepthTables* pShiftToDepth, const XnShiftToDepthConfig* pConfig)
{
	...
	XnUInt32 nShiftScale = (pConfig->nShiftScale / 10);
	XnUInt32 nMaxDepth = nShiftScale *pConfig->nDepthMaxCutOff;
	XnUInt32 nMinDepth = pConfig->nDepthMinCutOff *nShiftScale;
	nMaxDepth = XN_MIN(pConfig->nDeviceMaxDepthValue, nMaxDepth);
	...
	for (nIndex = 1; nIndex < pConfig->nDeviceMaxShiftValue; nIndex++)
	{
		...
		if (OpticsRelation == ProjectRightOfCMOS)
		{
			dFixedRefX = 2048 - ((nShiftValue + 2048) & 0xFFF);
		}
		else
		{
			dFixedRefX = ((nShiftValue + 2048) & 0xFFF) - 2048;
		}
		dFixedRefX /= 16.0;
		dDepth = pConfig->nShiftScale * dPlaneDsr / (1 + dPlanePixelSize * dFixedRefX / dPlaneDcl);
#endif
		// check cut-offs
