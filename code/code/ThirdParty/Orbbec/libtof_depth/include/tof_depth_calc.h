...
extern "C"
{
	...
	*/
	...
	/**
	* @brief		创建标定参数
	* @param[in]	buffer - 标定文件数据 buffer
	* @param[in]	buffer_size - 数据长度
	* @param[in]	hConfig - 滤波配置参数，用于校验标定参数的有效性
	* @return		成功 - 参数句柄，失败 - NULL
	*/
	TOF_API HANDLE CreateToFCaliParam(const uint8_t *buffer, int buffer_size, const HANDLE hConfig);
	...
	* @param[in]	hConfig - 滤波配置参数
	* @param[in]	hCalib - 标定参数
	* @return		0
	*/
	TOF_API int ResetPreRawphase(const HANDLE hConfig, const HANDLE hCalib);
	...
	* @param[out]	distort_coeff[5] - distort_coeff matrix 
	* @return		0 - 成功，其他 - 失败
	*/
	TOF_API int GetToFIntrinsic(const HANDLE hCali, float intrin[5], float distort_coeff[5]);

	/** 
	* @brief		算法接口，计算深度图、IR 图、幅值图
	...
	* @return		无
	*/
	TOF_API void StoreOutput(const char* dir, const char* fname, const HANDLE hConfig, const HANDLE hCali,
    ...
    * @return		积分时间
    */
    TOF_API int ExposureTimeCalcWithConfig(int input_t, const uint16_t *rawphase, const HANDLE hConfig);
