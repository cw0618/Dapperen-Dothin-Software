...
#define  LOGD(x...)  __android_log_print(ANDROID_LOG_INFO,"usbfs",x)
#define  LOGE(x...)  __android_log_print(ANDROID_LOG_ERROR,"usbfs",x)
...
static int get_config_descriptor(struct libusb_context *ctx, int fd,uint8_t config_index, unsigned char *buffer, size_t len);
#endif
...
#ifdef FREE_SELINUX
static int get_usbfs_device_descriptor_by_fd(struct libusb_device *dev, int fd)
{
	struct linux_device_priv *priv = _device_priv(dev);
	...
	if (!dev_buf) {
     ...
     if (!lseek(fd, 0, SEEK_SET)) {
        int r = read(fd, dev_buf, DEVICE_DESC_LENGTH);
        if (r < 0) {
            usbi_err(DEVICE_CTX(dev),"read descriptor failed ret=%d errno=%d", fd, errno);
            free(dev_buf);
            dev_buf=NULL;
            //close(fd);
            return LIBUSB_ERROR_IO;
...
#ifdef FREE_SELINUX 
static int get_usbfs_config_descriptor_by_fd(struct libusb_device *dev, int fd)
{   
    struct linux_device_priv *priv = _device_priv(dev);
    ...
    priv->config_descriptor = NULL;
     ...
     int active_config = usbfs_get_active_config(dev, fd);
     if (active_config == LIBUSB_ERROR_IO) {
        /* buggy devices sometimes fail to report their active config.
        /* assume unconfigured and continue the probing */
        usbi_warn(DEVICE_CTX(dev), "couldn't query active configuration, assumung unconfigured");
        LOGE("active_config LIBUSB_ERROR_IO");
     } else if (active_config < 0) {
        LOGE("active_config < 0");
	//close(fd);
	...
	int host_endian;
        int r= get_config_descriptor(DEVICE_CTX(dev), fd, i, tmp_buf, sizeof(tmp_buf));
	if (r < 0){
	    //close(fd);	
            return r;
        }
	if (tmp_buf[5] == active_config) {
	    idx = i;		
	}
     }
	...
	const char *sys_name)
{
	...
	if (!sysfs_can_relate_devices || detached || NULL == sys_name) {
		...
		if (!strncmp(dev_node, "/proc/bus/usb", 13)) {
	...
	struct discovered_devs **_discdevs)
{
	...
	while ((entry = readdir(buses))) {
		...
		if (usbdev_names) {
			...
			r = enumerate_device(ctx, &discdevs_new, busnum, (uint8_t) devaddr, NULL); 
			if (r < 0) {
				usbi_dbg("failed to enumerate dir entry %s", entry->d_name);
				continue;
			}
...
static int find_fd_by_name(char *file_name)
{
	...
	if(proc_fd == NULL)
	{
		LOGE("openni2 opendir DIR is a null pointer %d...",errno);
		return -errno;
	}
...
// *** PrimeSense patch for Android ***	
	...
	LOGE("openni2 op_open filename: %s, fd:%d", filename, hpriv->fd);
	// Fallback for regular non-java applications
	if (hpriv->fd == -1)
	{
		LOGE("openni2 op_open2...");
...
// *** PrimeSense patch for Android ***
	...
	if (hpriv->fd < 0) {
		if (errno == EACCES) {
			...
			LOGE("libusb couldn't open USB device %s: "
				"Permission denied.", filename);
			...
			LOGE("open failed, code %d errno %d", hpriv->fd, errno);
			return LIBUSB_ERROR_IO;
		}
	}
...
#ifdef FREE_SELINUX
static int op_get_configuration2(struct libusb_device_handle *handle,int fd)
{
      // get descriptor by fd
       //_device_handle_priv(handle);
      int ret= get_usbfs_device_descriptor_by_fd(handle->dev, fd);
	  if(ret!=LIBUSB_SUCCESS){
	     return ret;
	  }
      ret=get_usbfs_config_descriptor_by_fd(handle->dev, fd);
	  if(ret!=LIBUSB_SUCCESS){
	    return ret;
	  }
      return 0;
...
const struct usbi_os_backend linux_usbfs_backend = {
	...
	.open = op_open,
#if defined(FREE_SELINUX)
	.open2 = op_open2,
	.device2 = op_device2,
	.get_configuration2 = op_get_configuration2,
#endif	
	.close = op_close,
