...
#define  LOGD(x...)  __android_log_print(ANDROID_LOG_INFO,"sync",x)
#define  LOGE(x...)  __android_log_print(ANDROID_LOG_ERROR,"sync",x)
	...
	unsigned char *data, uint16_t wLength, unsigned int timeout)
{
	...
	{
		//Sometimes transfer - > buffer will be a null pointer.
		if(!transfer)
		{
			LOGE("Libusb Transfer is null...");
		...
		else
		{
			...
			{
				LOGE("Libusb Transfer buffer is null...");
				transfer->status = LIBUSB_TRANSFER_ERROR;
			}
			else
			{
