...
enum usbi_log_level ctx_level = LOG_LEVEL_NONE;
/**
...
#ifdef FREE_SELINUX
int API_EXPORTED libusb_get_configuration2(libusb_device *dev,int fd)
{
       if(usbi_backend->get_configuration2 == NULL) {
		/* Not supported on this platform */
		return NULL;
	}
	...
	_handle->claimed_interfaces = 0;
	memset(&_handle->os_priv, 0, priv_size);
	return usbi_backend->get_configuration2(_handle, fd);
}
#endif
/** \ingroup dev
	...
	const char *function, const char *format, va_list args)
{
	...
	if (ctx_level == LOG_LEVEL_NONE)
		return;
	if (level == LOG_LEVEL_WARNING && ctx_level < LOG_LEVEL_WARNING)
		return;
	if (level == LOG_LEVEL_INFO && ctx_level < LOG_LEVEL_INFO)
		return;
	if (level == LOG_LEVEL_DEBUG && ctx_level < LOG_LEVEL_DEBUG)
		return;
...
// *** PrimeSense patch for Android ***
	...
	message[len] = '\0';
	__android_log_write(ANDROID_LOG_INFO, "libusb", message);
// *** PrimeSense patch for Android ***
}
