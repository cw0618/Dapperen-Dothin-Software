    ...
    if (bResult)
    {
        ...
        do {
            ...
            for (nEPIdx = 0; nEPIdx < pUSBInterfaceDesc->bNumEndpoints; nEPIdx++)
            {
                ...
                //skip SuperSpeed Endpoint Companion
                if (*pBuf == 0x06)
                {
                    pBuf += 6;
...
XN_THREAD_PROC xnUSBReadThreadMain(XN_THREAD_PARAM pThreadParam)
{
    ...
    {
        xnLogError(XN_MASK_USB, "xnUSBReadThreadMain: CreateIoCompletionPort error!\n");
        return (XN_STATUS_ERROR);
    }
    ...
    for (;;)
    {
        ...
        if (bResult == FALSE)
        {
            xnLogVerbose(XN_MASK_USB, "Error (%d) occurred while reading the endpoint (0x%x)", GetLastError(), pThreadData->pEPHandle->nEndPointID);
