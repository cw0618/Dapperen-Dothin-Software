...
XN_C_API XnStatus xnLogSetAndroidOutput(XnBool bAndroidOutput)
{
	...
	if (bAndroidOutput)
	{
		...
		XN_IS_STATUS_OK(nRetVal);
		logData.AndroidWriter.m_logOutput = true;

	}
	else
	{
		logData.AndroidWriter.Unregister();
		logData.AndroidWriter.m_logOutput = false;
	}

	return XN_STATUS_OK;
}

XN_C_API XnStatus xnLogSetAndroidOutputRedirect(XnBool bAndroidOutput)
{
	XnStatus nRetVal = XN_STATUS_OK;
	...
	if (bAndroidOutput)
	{
		nRetVal = logData.AndroidWriter.Register();
		XN_IS_STATUS_OK(nRetVal);
		logData.AndroidWriter.m_logOutputRedirect = true;
	}
	else
	{
		logData.AndroidWriter.Unregister();
		logData.AndroidWriter.m_logOutputRedirect = false;
	}

	return XN_STATUS_OK;
}

XN_C_API XnStatus xnLogRegisterAndroidOutputRedirectCallback(XnLogRedirectEventHandler handler, void* pCookie, XnCallbackHandle* handle)
{
	XnStatus nRetVal = XN_STATUS_OK;
	LogData& logData = LogData::GetInstance();
	if (logData.AndroidWriter.m_logOutputRedirect)
	{
		nRetVal = logData.AndroidWriter.RegisterAndroidOutputRedirectCallback(handler, pCookie, handle);
		XN_IS_STATUS_OK(nRetVal);
	}
...
XN_C_API void xnLogUnRegisterAndroidOutputRedirectCallback(XnCallbackHandle handle)
{
	LogData& logData = LogData::GetInstance();
	if (logData.AndroidWriter.m_logOutputRedirect)
	{
		logData.AndroidWriter.UnRegisterAndroidOutputRedirectCallback(handle);
	}
}
