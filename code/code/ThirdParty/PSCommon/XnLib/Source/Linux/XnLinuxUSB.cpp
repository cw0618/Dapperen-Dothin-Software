...
#define LOGD(x...) __android_log_print(ANDROID_LOG_INFO, "XnLinuxUSB", x)
#define LOGE(x...) __android_log_print(ANDROID_LOG_ERROR, "XnLinuxUSB", x)
...
#define XN_USB_INTERFACE_DESCRIPTOR "openni"
...
typedef xnl::List<XN_USB_DEV_HANDLE> XnUSBOpenedDeviceList;
XnUSBOpenedDeviceList g_openedDevices;
...
XnStatus xnUSBPlatformSpecificInit()
{
    ...
    if (rc != 0)
    {
        xnLogError(XN_MASK_USB, "Initializing USB error %d ", rc);
        return (XN_STATUS_USB_INIT_FAILED);
    }
#ifdef FREE_SELINUX
    libusb_device *device = NULL;
    device = libusb_get_device2(g_InitData.pContext, m_usbpath);
    if (!device)
    {
        xnLogVerbose(XN_MASK_USB, "libusb_get_device2 failed !");
        return (XN_STATUS_USB_INIT_FAILED);
    }
    libusb_get_configuration2(device, m_fd);
#endif
    ...
    //	libusb_set_debug(g_InitData.pContext, 4);
...
XN_C_API XnStatus xnUSBEnumerateDevices(XnUInt16 nVendorID, XnUInt16 nProductID, const XnUSBConnectionString **pastrDevicePaths, XnUInt32 *pnCount)
{
    ...
    for (ssize_t i = 0; i < nDeviceCount; ++i)
    {
        ...
        {
#if (XN_PLATFORM == XN_PLATFORM_ANDROID_ARM) || defined(XN_PLATFORM_MACOSX_XCODE) || defined(XN_PLATFORM_IOS)
            LOGE("%s:%d 0x%x: 0x%x", __FILE__, __LINE__, desc.idVendor, desc.idProduct);
#endif
...
XN_C_API XnStatus xnUSBQueryInterfaces(libusb_device_handle* handle, libusb_device* pDevice, XnUInt8* piInterface, XnUInt8* pnInterface, XnUInt8* pnAltInterface)
{
    /// Validate parameters.
    XN_VALIDATE_USB_INIT();
    XN_VALIDATE_DEVICE_HANDLE(pDevice);
    ...
    if (0 != rc)
    {
        xnLogError(XN_MASK_USB, "Failed to query usb configuration...");
        return (XN_STATUS_USB_CONFIG_QUERY_FAILED);
    }

    /// Query all interfaces.
    XnChar strDesc[64] = { 0 };
    const libusb_interface* pInterface = NULL;
    const libusb_interface_descriptor* pInterfaceDesc = NULL;
    for (int32_t i = 0; i < (int32_t)pConfig->bNumInterfaces; ++i)
    {
        pInterface = &pConfig->interface[i];
        for (int32_t j = 0; j < pInterface->num_altsetting; ++j)
        {
            xnOSMemSet(strDesc, 0, sizeof(strDesc));
            pInterfaceDesc = &pInterface->altsetting[j];
            XnInt32 len = libusb_get_string_descriptor_ascii(handle, pInterfaceDesc->iInterface, (uint8_t*)strDesc, sizeof(strDesc));
            if (len <= 0)
            {
                xnLogWarning(XN_MASK_USB, "Failed to get usb interface (%d) descriptor...", pInterfaceDesc->iInterface);
                continue;
            }

            strDesc[len] = '\0';
            xnOSStrToLower(strDesc);
            xnLogInfo(XN_MASK_USB, "USB interface (%d) descriptor: %s", pInterfaceDesc->iInterface, strDesc);

            if (NULL != xnOSStrStr(strDesc, XN_USB_INTERFACE_DESCRIPTOR))
            {
                *pnInterface = (XnUInt8)i;
                *pnAltInterface = (XnUInt8)j;
                *piInterface = pInterfaceDesc->iInterface;
    ...
    libusb_free_config_descriptor(pConfig);
    xnLogError(XN_MASK_USB, "Failed to query usb interface...");
...
XN_C_API XnStatus xnUSBSetInterface(XN_USB_DEV_HANDLE pDevHandle, XnUInt8 nInterface, XnUInt8 nAltInterface)
{
    ...
    for (XnUSBOpenedDeviceList::Iterator it = g_openedDevices.Begin(); it != g_openedDevices.End(); ++it)
    {
        if (*it == pDevHandle && g_nSetCount > 0)
        {
            if (nInterface != pDevHandle->nInterface || nAltInterface != pDevHandle->nAltSetting)
            {
                xnLogInfo(XN_MASK_USB, "The usb interface setting value [%d, %d] not match with currently value [%d, %d], counter = %d",
                          nInterface, nAltInterface, pDevHandle->nInterface, pDevHandle->nAltSetting, g_nSetCount);
                return XN_STATUS_OK;
            }
        }
    }
    ...
    if (rc != 0)
    {
        xnLogInfo(XN_MASK_USB, "Failed to set usb interface: nInterface = %d, nAltInterface = %d, count = %d", nInterface, nAltInterface, g_nSetCount);
...
XN_C_API XnStatus xnUSBOpenDeviceImpl(libusb_device *pDevice, XN_USB_DEV_HANDLE *pDevHandlePtr)
{
    ...
    {
    case 0x300:
        nSpeed = LIBUSB_SPEED_SUPER;
        break;
    case 0x200:
        nSpeed = LIBUSB_SPEED_HIGH;
        break;
    case 0x110:
        nSpeed = LIBUSB_SPEED_FULL;
        break;
    default:
        break;
    }
    ...
    // open device
#ifdef FREE_SELINUX
    rc = libusb_open2(pDevice, &handle, m_fd);
#else
    rc = libusb_open(pDevice, &handle);
#endif

    // now check if open failed
    xnLogInfo(XN_MASK_USB, "Usb device opened status %d", rc);
    if (rc != 0)
    {
    ...
    XnUInt8 iInterface = 0;
    XnUInt8 nInterface = 0;
    XnUInt8 nAltInterface = 0;
    rc = xnUSBQueryInterfaces(handle, pDevice, &iInterface, &nInterface, &nAltInterface);
    if (XN_STATUS_OK != rc)
        xnLogWarning(XN_MASK_USB, "Default interface (0) would be set!");

    // in any case, unref the device (we don't need it anymore)
    libusb_unref_device(pDevice);
    pDevice = NULL;

    if (nSpeed == LIBUSB_SPEED_SUPER)
    {
        // set for the first (and only) configuration (this will perform a light-weight reset)
        rc = libusb_set_configuration(handle, 1);
        xnLogInfo(XN_MASK_USB, "USB set configuration status %d", rc);
        if (rc != 0)
        {
            libusb_close(handle);
    ...
    rc = libusb_claim_interface(handle, nInterface);
    if (rc != 0)
    {
        libusb_close(handle);
        xnLogError(XN_MASK_USB, "Failed to claim usb interface (%d), error code (%d)...", nInterface, rc);
        return (XN_STATUS_USB_SET_INTERFACE_FAILED);
    }
    ...
    pDevHandle->iInterface = iInterface;

    // mark the device is of high-speed
    switch (nSpeed)
    {
    case LIBUSB_SPEED_SUPER:
        pDevHandle->nDevSpeed = XN_USB_DEVICE_SUPER_SPEED;
        xnLogInfo(XN_MASK_USB, "USB is Super Speed");
        break;
    case LIBUSB_SPEED_HIGH:
        pDevHandle->nDevSpeed = XN_USB_DEVICE_HIGH_SPEED;
        xnLogInfo(XN_MASK_USB, "USB is high Speed");
        break;
    case LIBUSB_SPEED_FULL:
        pDevHandle->nDevSpeed = XN_USB_DEVICE_FULL_SPEED;
        xnLogInfo(XN_MASK_USB, "USB is full Speed");
        break;
    case LIBUSB_SPEED_LOW:
        pDevHandle->nDevSpeed = XN_USB_DEVICE_LOW_SPEED;
        xnLogInfo(XN_MASK_USB, "USB is low Speed");
        break;
    default:
        //pDevHandle->nDevSpeed = XN_USB_DEVICE_HIGH_SPEED;
    ...
    rc = xnUSBSetInterface(pDevHandle, nInterface, nAltInterface);
    if (XN_STATUS_OK != rc)
    {
    ...
    g_openedDevices.AddLast(pDevHandle);
    return (XN_STATUS_OK);
}
...
XN_C_API XnStatus xnUSBCloseDevice(XN_USB_DEV_HANDLE pDevHandle)
{
    ...
    g_openedDevices.Remove(pDevHandle);
...
/** Checks if any transfer of the thread is submitted. */
XnBool xnIsAnyTransferSubmitted(XnUSBReadThreadData *pThreadData)
{
    for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i)
    {
        if (pThreadData->pBuffersInfo[i].bIsSubmit)
            return (TRUE);
    }
...
XN_THREAD_PROC xnUSBReadThreadMain(XN_THREAD_PARAM pThreadParam)
{
    ...
    for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i)
    {
        ...
        pBufferInfo->bIsSubmit = TRUE;
        int rc = libusb_submit_transfer(pTransfer);
        if (rc != 0)
        {
            pBufferInfo->bIsQueued = FALSE;
            pBufferInfo->bIsSubmit = FALSE;
            pBufferInfo->nLastStatus = LIBUSB_TRANSFER_ERROR;
    ...
    while (TRUE)
    {
        for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i)
        {
            ...
            if (pThreadData->bKillReadThread && !xnIsAnyTransferQueued(pThreadData) && !xnIsAnyTransferSubmitted(pThreadData))
            {
                XN_THREAD_PROC_RETURN(XN_STATUS_OK);
            }

            XnUSBBuffersInfo *pBufferInfo = &pThreadData->pBuffersInfo[i];
            if (pThreadData->bKillReadThread && !pBufferInfo->bIsQueued && !pBufferInfo->bIsSubmit)
            {
                continue;
            }
...
#ifdef FREE_SELINUX
void xnUSBPlatformSetUsbParam(int fd, const char *usbpath)
{
    m_fd = fd;
    m_usbpath = const_cast<char *>(usbpath);
}
#endif
