...
public class CoordinateConverter {
    ...
    public static Point2D<Integer> convertC2DCoordinateByIntrinsic(final VideoStream depthStream,
                                                                   int colorX, int colorY, short depthZ) {
        OutArg<Integer> x = new OutArg<Integer>();
        OutArg<Integer> y = new OutArg<Integer>();
        NativeMethods.checkReturnStatus(NativeMethods.oniCoordinateConverterC2D(
                depthStream.getHandle(), colorX, colorY, depthZ, x, y));
        int depthX = x.mValue;
        int depthY = y.mValue;
        return new Point2D<Integer>(depthX, depthY);
    }

    public static Point2D<Integer> convertD2CCoordinateByIntrinsic(final VideoStream depthStream,
                                                                   int depthX, int depthY, short depthZ) {
        OutArg<Integer> x = new OutArg<Integer>();
        OutArg<Integer> y = new OutArg<Integer>();
        NativeMethods.checkReturnStatus(NativeMethods.oniCoordinateConverterD2C(
                depthStream.getHandle(), depthX, depthY, depthZ, x, y));
        int colorX = x.mValue;
        int colorY = y.mValue;
        return new Point2D<Integer>(colorX, colorY);
    }
