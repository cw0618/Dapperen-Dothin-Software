...
public class OpenNIHelper {
	...
	public static interface DeviceOpenListener {
		 ...
		 */
		public abstract void onDeviceOpenFailed(String msg);
	}
				...
				synchronized (this) {
					...
					if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
						...
						try {
							...
							mDeviceOpenListener.onDeviceOpenFailed("Can't open device though permission was granted: " + ex.getMessage());
						}
					} else {
						Log.e(TAG, "Permission denied for device");
						mDeviceOpenListener.onDeviceOpenFailed("Permission denied for device");
					}
				}
			}
		}
	...
	private void openDevice(UsbDevice device) {
		try {
			...
			if(mUsbDeviceConnection == null) {
				mDeviceOpenListener.onDeviceOpenFailed(" usbserver open device failed");
			}else {
				int usbfd=mUsbDeviceConnection.getFileDescriptor();
				String path=device.getDeviceName();
				Log.d(TAG, "usbfd:"+usbfd+"usbpath:"+path);
				OpenNI.SetUsbParam(usbfd, path);
				OpenNI.initialize();
				mDeviceOpenListener.onDeviceOpened(device);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			mDeviceOpenListener.onDeviceOpenFailed("open device error: " + ex.getMessage());
		}
	}

	public int getUsb_fd(UsbDevice device) {
		UsbManager manager = ((UsbManager) mAndroidContext.getSystemService(Context.USB_SERVICE));
		mUsbDeviceConnection = manager.openDevice(device);
		if (mUsbDeviceConnection == null) {
			return -1;
		}
		return mUsbDeviceConnection.getFileDescriptor();
	}
	...
	public void setDeviceOpenListener(DeviceOpenListener listener)	{
		mDeviceOpenListener=listener;
	}
