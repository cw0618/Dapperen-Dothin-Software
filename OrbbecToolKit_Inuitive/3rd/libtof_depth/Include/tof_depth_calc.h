...
extern "C"
{
	...
	*/
	...
	/**
	* @brief			获取模组SN号
	* @param[in]	    buffer - 标定参数
	* @param[out]       dev_sn - 模组SN号
	* @return			0
	*/
	TOF_API int GetDeviceSN(void* buffer, char* dev_sn);
	/*
	* @brief			获取模组内参
	* @param[in]	    file_path - 标定文件路径
	* @param[out]       ir_intrin - 模组内参
	* @return			0
	*/
	TOF_API int GetIntrinsic(const char *file_path, TofIntrinsic &ir_intrin);
	/**
	* @brief			释放句柄
	...
	* @return		0
	*/
	TOF_API int ResetPreRawphase(const HANDLE hConfig);
	...
	* @return		0 - 成功，其他 - 失败
	*/
	TOF_API int DepthCalc(const int16_t *rawphase, const ToFDevInfo *dInfo, const HANDLE hConfig, const HANDLE hCali,
    ...
    * @return		积分时间
    */
    TOF_API int ExposureTimeCalcWithConfig(int input_t, const int16_t *rawphase, const HANDLE hConfig, const int *roi);
