...
#define XN_PS_DEVICE_MAX_STRING_LENGTH 256
#define XN_USB_MAX_FILE_NAME_LENGTH 32
...
enum
{
	...
	XN_MODULE_PROPERTY_EEPROM_STATE = 0x1080E010,  // EEPROM
    ...
    XN_MODULE_PROPERTY_ENABLE_SUBTRACT_BG = 0x1080FFF2,
    XN_MODULE_PROPERTY_STATE_SUBTRACT_BG = 0x1080FFF3,

    XN_MODULE_PROPERTY_USB_GRNERAL_FILE = 0x1080FFF4,
    XN_MODULE_PROPERTY_PLATFORM_VERSION = 0x1080FFF5,
    XN_MODULE_PROPERTY_PLATFORM_SDK_VERSION = 0x1080FFF6,
    XN_MODULE_PROPERTY_TOF_FREQ_MODE = 0x1080FFF7,
    XN_MODULE_PROPERTY_START_SERVICE = 0x1080FFF8,
    XN_MODULE_PROPERTY_GENERAL_SERIAL_NUMBER = 0x1080FFF9,
    XN_MODULE_PROPERTY_SENSOR_ID = 0x1080FFFA,
    XN_MODULE_PROPERTY_SEND_COMMAND = 0x1080FFFE,
    XN_MODULE_PROPERTY_QUERY_DEVICE_TIMESTAMP = 0x1080FFFF,

    //Motor
    XN_MODULE_PROPERTY_MOTOR_TEST = 0x10810001,
    XN_MODULE_PROPERTY_MOTOR_POSITION = 0x10810002,
    XN_MODULE_PROPERTY_MOTOR_STATUS = 0x10810003,
	XN_MODULE_PROPERTY_MOTOR_TEST_COUNT = 0x10810004,
	XN_MODULE_PROPERTY_MOTOR_RUN_TIME = 0x10810005,
    XN_MODULE_PROPERTY_MOTOR_UPDOWN_STATE = 0x10810007,
    XN_MODULE_PROPERTY_MOTOR_UPDOWN_TIME = 0x10810008,
    XN_MODULE_PROPERTY_MOTOR_UPDOWN_CONTROL = 0x10810009,
    XN_MODULE_PROPERTY_MOTOR_FEATURE = 0x10810011,

    XN_MODULE_PROPERTY_JAVA_VM = 0x10810006,
...
typedef enum
{
    ...
    XN_IO_IMAGE_FORMAT_COMPRESSED_MJPEG = 8,
    XN_IO_IMAGE_FORMAT_H264 = 9,
} XnIOImageFormats;
...
typedef enum
{
    ...
    XN_IO_DEPTH_FORMAT_UNCOMPRESSED_DEPTH_16_BIT = 5,
} XnIODepthFormats;
...
typedef enum
{
    ...
    XN_IO_IR_FORMAT_COMPRESSED_MJPEG = 5,
} XnIOIRFormats;

typedef enum
{
    XN_IO_PHASE_FORMAT_UNCOMPRESSED_16_BIT_EXTRA_LINE_1 = 0,
    XN_IO_PHASE_FORMAT_COMPRESSED_10_BIT_ALIGN_256_EXTRA_LINE_1 = 1,
    XN_IO_PHASE_FORMAT_COMPRESSED_10_BIT_EXTRA_LINE_1 = 2,
    XN_IO_PHASE_FORMAT_MIPI_10_BIT_EXTRA_LINE_1 = 3,
} XnIOPhaseFormats;
...
typedef enum
{
    ...
    XN_IO_AI_FORMAT_DEPTH_IR = 0x0040,
    XN_IO_AI_FORMAT_DEPTH_PHASE = 0x0080,
    XN_IO_AI_FORMAT_FACE = 0x0100,
    XN_IO_AI_FORMAT_GESTURE = 0x0200,
} XnIOAIFormats;
...
typedef enum
{
    XN_USB_FILE_RECEIVE_IDLE,
    XN_USB_FILE_RECEIVING,
    XN_USB_FILE_RECEIVE_FINISH,
    XN_USB_FILE_RECEIVE_TIMEOUT,
    XN_USB_FILE_RECEIVE_REJECT,
} XnUsbFileTransferDeviceStatus;

typedef enum
{
    XN_USB_FILE_SUFFIX_NONE,
    XN_USB_FILE_SUFFIX_TEXT,
    XN_USB_FILE_SUFFIX_BINARY,
} XnUsbFileSuffix;

typedef enum
{
    XN_USB_FILE_CATEGORY_NONE,
    XN_USB_FILE_CATEGORY_FW,
    XN_USB_FILE_CATEGORY_ROM,
    XN_USB_FILE_CATEGORY_D2C,
    XN_USB_FILE_CATEGORY_CALIB,
    XN_USB_FILE_CATEGORY_FILTER,
    XN_USB_FILE_CATEGORY_COMMON,
} XnUsbFileCategory;

typedef struct
{
    char name[XN_USB_MAX_FILE_NAME_LENGTH];
    uint32_t size;
    uint16_t suffix;   ///< @see XnUsbFileSuffix
    uint16_t category; ///< @see XnUsbFileCategory
    uint32_t uid;
    uint16_t crc;
    uint32_t reserve;
} XnUsbFileAttributes;
...
typedef struct
{
    XnUsbFileAttributes attributes;
    uint8_t* pContent;
} XnUsbGeneralFile;
...
{
    XN_FIGURE_THIN,
    XN_FIGURE_OVERWEIGHT,
    XN_FIGURE_MEDIUM_BUILT,
} XnFigureTypes;

typedef struct
{
    uint16_t id;                                   ///< Reserve
    uint16_t figure;                               ///< Reserve (@see XnFigureTypes)
    float ratio;
    float height;
    float waist;
    ...
    uint32_t points[ONI_BODY_POINT_NUM * 2];       ///< Body Points
} XnBodyShape;
